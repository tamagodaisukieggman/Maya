...
## 1.21 improve Bevel Manager
## 1.22 add live Drag Position and shift/ctrl change rotation
## 1.23 bug fix and improve NonBevel function in bevel Manager
## 1.24 check liveSnap object and smapleLocator is not exist when create cutter,small viusal improvement.
...
## 1.43 improve control slider , but undo still not work well
...
## 1.53 improved UI, better show between operation types, vis between different base mesh grp, assign color for different mesh grp
...
def bt_speedCutUI():
    if mc.window("bt_SpeedCutWin", exists = True):
        mc.deleteUI("bt_SpeedCutWin")
    jwSpeedCutWin = mc.window("bt_SpeedCutWin",title = "speedCut 1.58",w = 300,h = 780, mxb = False, s = 1 ,bgc = [0.2, 0.2, 0.2  ])
    mc.columnLayout()
    ...
    mc.text(l ='')
    mc.button('setBaseButton', l= "Set", c = 'checkBaseMeshList(),setCutterBaseMesh(),baseMeshColorInitail(),updateVisLayer()' ,  bgc = [0.28,0.28,0.28] )
    mc.setParent( '..' )
    ...
    mc.text(l ='',h=20)
    mc.iconTextButton('preBevelButton',  w=40,style='textOnly', l= "Bevel Manager", rpt = True, c = 'bt_speedCutBevelManagerSetup()', bgc = [0.14, 0.14, 0.14] )
    mc.separator( height=15, style='none' )
    mc.setParent( '..' )
    mc.separator( height=15, style='none' )
    mc.showWindow(jwSpeedCutWin)
    mc.window("bt_SpeedCutWin",e=True,w = 320,h = 770)
...
def cutter2NewMeshGrp():
    ...
    if len(selCut) == 1:
        ...
        if getOPType != 'union':
            ...
            mc.parent(combineShape[0],meshNameGrp)
            mc.rename(meshName)
            mc.parent(w=True)
            ...
            mc.connectAttr((preCutShapeNode[0]+'.outMesh'), (shadowMesh[0]+'_myInter.inputPoly[0]'),f=True)
            mc.connectAttr((preCutShapeNode[0]+'.worldMatrix[0]'), (shadowMesh[0]+'_myInter.inputMat[0]'),f=True)
            ...
            mc.setAttr((shadowMesh[0]+'_myInter.operation'),2)
            mc.select(selCut)
            mc.sets(name = (shadowMesh[0] + 'Shadow'), text= (shadowMesh[0] + 'Shadow'))
            showAllCutter()
            mc.connectAttr((shadowMesh[0]+'_myInter.output'), (shadowMesh[0]+'_preSubBoxShape.inMesh'),f=True)
            baseMeshColorInitail()
        else:
...
def bt_speedCutBevelManagerSetup():
    beseMesh = mc.optionMenu('baseMeshMenu', query = True, v = True)
    if len(beseMesh)>0:
        bt_speedCutBevelManagerUI()
        bakeCutter('all')
...
def bt_speedCutBevelManagerUI():
    if mc.window("bt_speedCutBevelManager", exists = True):
        mc.deleteUI("bt_speedCutBevelManager")
    bt_speedCutBevelManager = mc.window("bt_speedCutBevelManager",title = "speedCut Bevel Manager",w = 400,h = 380, mxb = False, s = 1, bgc = [0.2, 0.2, 0.2  ])
    mc.columnLayout()
    ...
    mc.text(l ='')
    mc.separator( height=15, style='none' )
    mc.showWindow(bt_speedCutBevelManager)
    obNum = mc.scriptJob ( p = 'bt_speedCutBevelManager', event = ["SelectionChanged", reselctList])
...
def baseMeshColorUpdate():
    ...
    if shadeEng:
        ...
        if materials[0] == (beseMesh+'_Shader'):
            ...
            colorR = random.uniform(0.4,0.75)
            colorG = random.uniform(0.4,0.75)
            colorB = random.uniform(0.4,0.75)
            mc.setAttr ((beseMesh +'_Shader.color'), colorR, colorG,colorB, type = 'double3' )
            mc.sets((beseMesh+'_bool'), e=True, forceElement = (beseMesh+'_ShaderSG'))
    else:
        mc.sets((beseMesh+'_bool'), e=True, forceElement = 'initialShadingGroup')

def baseMeshColorInitail():
    checkBaseMeshList()
    beseMeshList = mc.optionMenu('baseMeshMenu', query = True, ils = True)
    beseMesh = mc.optionMenu('baseMeshMenu', query = True, v = True)
    if len(beseMeshList) == 1:
        pass
    else:
        if mc.objExists(beseMesh+'_bool'):
            shdSG = []
            if mc.objExists(beseMesh+'_Shader') == 0:
                shd = mc.shadingNode('lambert', name=(beseMesh+'_Shader'), asShader=True)
                shdSG = mc.sets(name=(beseMesh+'_ShaderSG'), empty=True, renderable=True, noSurfaceShader=True)
                ...
                chooceColor = 200 + (50*n)
                if chooceColor > 328:
                    chooceColor = chooceColor -328
                getColor = hsv2rgb(int(chooceColor),0.2,0.5)
                mc.setAttr ((shd +'.color'), getColor[0],getColor[1],getColor[2], type = 'double3' )
            mc.sets((beseMesh+'_bool'), e=True, forceElement = (beseMesh+'_ShaderSG'))
