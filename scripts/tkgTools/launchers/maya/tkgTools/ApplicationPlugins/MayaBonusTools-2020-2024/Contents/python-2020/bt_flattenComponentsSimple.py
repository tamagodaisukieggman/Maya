...
def bt_flattenComponentsSimple():
    ...
    vertsShapeNode = ls(vertList[0], o=True)   
    vertObject = string.split (vertList[0], '.')[0]
    
    if (nodeType(vertsShapeNode) == 'mesh'):    
        vertsObject = listRelatives(vertsShapeNode,p=1)
    else:
    ...
    cmds.setAttr( snapPlane+'.visibility', 0)
    ...
    delete (snapPlaneParent)   
    hilite (vertObject)
    
    print ('Components flattened')

#bt_flattenComponentsSimple()
