#Interactive Tutorial Creator app (V2.0) by Matthew Chan
...
V 2.0 (Compatibility note!)
- Added the ability to place overlays relative to objects in the Viewport.
- In previous versions, OS font scaling would displace word bubbles, causing the "tail" to no longer point at the same spot.  To fix that, this version of the script places all bubbles relative to the tail rather than by the top-left corner.  Now, regardless of the font size, the tail will always be in the same spot.  
- Compatibility branch: Due to the changes above, this script is incompatible with tutorials created by previous versions of the script.

V 1.2
- Fixed a bug that broke the "Import curent camera + values" button when the camera had a specific kind of name
- Changed the default title of the controller to the corresponding stage name, and the description to read "Press <b>Next</b>"
- Fixed a bug that caused the controller to stay hidden when using "Play from Selected".

V 1.1

- Fixed a bug that broke the "Import current camera + values" button on the Initialize stage component
- Fixed a bug that caused text overlay previews to always appear centered when "Relative To" was set to "UI Code". (Note the actual placement was always working, just the preview was broken)
- Fixed a bug that caused an endless loop in the background if two Time Slider bookmarks were created in sequence without the tutorial being played through between.  (Note the original bug did not prevent user interaction and could be fixed by simply running the tutorial once)
...
#Return the size of the curent speech bubble loaded into the controller attributes
def getBubbleSize(html_text):
    x_offset = 0
    y_offset = 0
    t_manager = mo.maya.overlayManager()

    #populate bubble with text
    od = mo.overlayDef.OverlayDef()
    overlay = t_manager.createOverlay(od)
    ...
    overlay_size = QSize(label.sizeHint().width(), label.sizeHint().height())
       	        
    t_manager.deleteOverlay(overlay)

    return overlay_size

#Gets the position of the arrow tip relative to the top-left corner of the speech bubble
def getArrowTipCoord(html_text):
    x_offset = 0
    y_offset = 0
    t_manager = mo.maya.overlayManager()
    ...
    if label.sizeHint().width() > overlay_width:    
        label.setMaximumWidth(overlay_width)
        #label.setFixedWidth(overlay_width)
        label.setWordWrap(True)
    else:
        label.setWordWrap(False)
    ...
    t_dir = cmds.optionMenu("text_overlay_tail_dir", q=True, v=True)
    t_pos = cmds.floatSliderGrp("text_overlay_tail_pos", q=True, v=True)
    ...
    if(t_dir == "top"):
        ...
        x_offset = (-1*overlay_size.width()*(t_pos*0.97))-((0.5-t_pos)*margin_width)-arrow_width
    elif(t_dir == "left"):
        #overlay.addSpeechBubbleTail(mo.enums.Direction.Left, t_pos)
        y_offset = (-1*overlay_size.height()*(t_pos*0.9))-((0.5-t_pos)*margin_height)-arrow_width
    elif(t_dir == "bottom"):
        #overlay.addSpeechBubbleTail(mo.enums.Direction.Bottom, t_pos)
        x_offset = (-1*overlay_size.width()*(t_pos*0.97))-((0.5-t_pos)*margin_width)-arrow_width
        y_offset = -1*overlay_size.height()-margin_height-arrow_height
    else:
        #overlay.addSpeechBubbleTail(mo.enums.Direction.Right, t_pos)
        x_offset = (-1*overlay_size.width())-arrow_height
        y_offset = (-1*overlay_size.height()*(t_pos*0.9))-((0.5-t_pos)*margin_height)-arrow_width
    
    #t_manager.showAll()  	        
    t_manager.deleteOverlay(overlay)

    return QSize(x_offset, y_offset)
...
def displayPreviewOverlay(*args):
    ...
    if rel_to == "UI code or scene object / component":
        rel_to = cmds.textField("text_overlay_rel_to_custom", q=True, text=True)
    ...
    if not rel_to in cardinal_points :
        ...
        try:
            ...
            point3d = cmds.xform(rel_to, q=True, t=True, ws=True)
            if point3d:    
                curCamera=cmds.modelEditor("modelPanel4",q=True,av=True,cam=True)
                height = cmds.control('modelPanel4', q=True, height=True)
                width= cmds.control('modelPanel4', q=True, width=True)
                coordinates = mo.pointWorldToCam(curCamera, (point3d[0], point3d[1], point3d[2]), (width, height)) 

                object_offset_x = x_offset + coordinates[0]/mo.utils.DPIScale(1)
                object_offset_y = y_offset + coordinates[1]/mo.utils.DPIScale(1)
			
                icon_panel_height = cmds.layout(cmds.modelPanel("modelPanel4", q=True, bl=True), q=True, h=True)
                object_offset_y = object_offset_y + icon_panel_height
			
                con_browse=mo.getUIPosition("modelPanel4")
                UI_success = True
            else:
    ...
    if (UI_success):
        x_offset = con_browse.x()+object_offset_x
        y_offset = con_browse.y()+object_offset_y
    ...
    if not dialog_style:
        ...
        if (UI_success):
            arrow_tip = getArrowTipCoord(text)
            
        offset = QSize(x_offset+arrow_tip.width(), y_offset+arrow_tip.height())
        ...
        #determine if bubble needs to be wrapped (i.e. too big)
        label.setWordWrap(False)
        overlay_width_scaled = overlay_width
...
def tsCreate(old_bookmark_list, selected_node, old_connected_bookmark):
    ...
    if new_bookmarks:
        ...
        if new_connected_bookmark != latest_bookmark:
            ...
            if cmds.text("step_range_text", exists=True):
                cmds.deleteUI("step_range_text")
                
            if cmds.intFieldGrp("step_frame_range", exists=True):
...
def placeOverlay(*args):
    ...
    def updatePreviewOverlay(*args):
        ...
        window_height = cmds.window("positioner_window", q=True, height=True)
        ...
        maya_window_top_right = [maya_window_top_left[0]+maya_window_height, maya_window_top_left[1]+maya_window_width]
        maya_window_bottom_left = [maya_window_top_left[0]+maya_window_height, maya_window_top_left[1]]
        maya_window_bottom_right = [maya_window_top_left[0], maya_window_top_left[1]+maya_window_width]
        
        x_offset = 0
        y_offset = 0
        placer_offset = 60
        html_text =  cmds.textField("text_overlay_html_text", q=True, text=True)
        overlay_size = getBubbleSize(html_text)
        
        
        if rel_to == "topleft":
            
            x_offset = (window_pos[1] - maya_window_top_left[1])/mo.utils.DPIScale(1)
            y_offset = (window_pos[0] - maya_window_top_left[0] - 25 - overlay_size.height())/mo.utils.DPIScale(1)
           
        elif rel_to == "top":
            x_offset = (window_pos[1] - maya_window_top_left[1] - maya_window_width / 2 + window_width / 2)/mo.utils.DPIScale(1)
            y_offset = (window_pos[0] - maya_window_top_left[0] - 25 - overlay_size.height())/mo.utils.DPIScale(1)
        
        elif rel_to == "topright":
            x_offset = (maya_window_top_right[1]-window_pos[1] - window_width)/mo.utils.DPIScale(1)
            y_offset = (window_pos[0] - maya_window_top_left[0] - 25 - overlay_size.height())/mo.utils.DPIScale(1)
            
        elif rel_to == "left":
            x_offset = (window_pos[1] - maya_window_top_left[1])/mo.utils.DPIScale(1)
            y_offset = (window_pos[0] - maya_window_top_left[0] - maya_window_height/2 - overlay_size.height())/mo.utils.DPIScale(1)
            ...
            x_offset = (window_pos[1] - maya_window_top_left[1])/mo.utils.DPIScale(1)
            y_offset = (maya_window_bottom_left[0]-window_pos[0] - 25 +overlay_size.height())/mo.utils.DPIScale(1)
        ...
        elif rel_to == "UI code or scene object / component":
            rel_to = cmds.textField("text_overlay_rel_to_custom", q=True, text=True)
            con_browse=[]
            object_offset_x=0
            object_offset_y=0
            try:
                ...
                if point3d:    
                    ...
                    object_offset_x = x_offset + (coordinates[0]+overlay_size.width()/2)/mo.utils.DPIScale(1)
                    object_offset_y = y_offset + coordinates[1]/mo.utils.DPIScale(1)
            ...
            if con_browse:
                ...
                x_offset = (window_pos[1] - maya_window_top_left[1] - con_browse.x() - object_offset_x + overlay_size.width())/mo.utils.DPIScale(1)
                y_offset = (window_pos[0] - maya_window_top_left[0] - con_browse.y() - object_offset_y - overlay_size.height() - 25 )/mo.utils.DPIScale(1) 
             
            tail_dir = cmds.optionMenu("text_overlay_tail_dir", q=True, v=True)
            if tail_dir == "bottom":
                y_offset = y_offset + overlay_size.height()/mo.utils.DPIScale(1)
            elif tail_dir == "left":
                x_offset = x_offset - (overlay_size.width()+window_width)/2/mo.utils.DPIScale(1)
                y_offset = y_offset + (overlay_size.height()-25)/mo.utils.DPIScale(1)
            elif tail_dir == "right":
                x_offset = x_offset - (window_width-overlay_size.width())/2/mo.utils.DPIScale(1)
                y_offset = y_offset + (overlay_size.height()-25)/mo.utils.DPIScale(1)     
...
def importCamera(*args):
    ...
    camera_shape = cmds.listRelatives(curr_camera[1])[0]
    coi = cmds.getAttr(camera_shape+".centerOfInterest") 
...
def buildEditPane(*args):
    ...
    if node_type == "script":
        ...
        if selected_type == "initialize":
            ...
            cmds.optionMenu("setup_shot_camera", parent = "UI_edit_pane", cc=partial(injectData, selected_node, [15]))
            ...
            cmds.scrollField("setup_code", wordWrap=False, parent="UI_edit_pane", text=setup_code, cc=partial(injectData, selected_node, [21]))
            ...
            cmds.menuItem("UI code or scene object / component") 
            ...
            #9 UI code     
            cmds.text("UI Code | Scene object/component ", parent="UI_edit_pane")
...
def addStepNodes(*args):
    ...
    if is_controller_update:
        ...
        #Rename the controller to the current stage number
        parsed_input_list = cmds.getAttr(last_update_controller+".before").split('#%INPUT%')    
        parsed_input_list[1] = '\ntitle = """'+new_stage+'"""\n'   
        reassembled_input = '' 
        for x in parsed_input_list[:-1]:
            reassembled_input = reassembled_input + x + '#%INPUT%'
        reassembled_input = reassembled_input + parsed_input_list[len(parsed_input_list)-1]    
        cmds.setAttr(last_update_controller+".before", reassembled_input, type="string")
...
def playFromCurrent(*args):
    ...
    cmds.evalDeferred("cmds.stage('"+current_stage+"', e=True, a=True)")

    #Find the current stage number and then increment the progress bar appropriately
    def findCurrentStageNum(input_stage):
        count = 0
        current_stage = input_stage
        
        while cmds.listConnections(current_stage+".previousState"):
            count = count + 1
            current_stage = cmds.listConnections(current_stage+".previousState")[0]
           
        return count-1
    
    stage_num = findCurrentStageNum(current_stage)
    cmds.evalDeferred("mo.g_overlay_manager.controller.progressBar().setCurrentStep("+str(stage_num)+")")

    cmds.evalDeferred("mo.g_overlay_manager.controller.showAndRaise()")
...
if not rel_to in cardinal_points :
	try:
		...
		if point3d:    
			...
			x_offset = x_offset + coordinates[0]/mo.utils.DPIScale(1)
			y_offset = y_offset + coordinates[1]/mo.utils.DPIScale(1)
			
			icon_panel_height = cmds.layout(cmds.modelPanel("modelPanel4", q=True, bl=True), q=True, h=True)
			y_offset = y_offset + 2*icon_panel_height
...
#%THISID%
this = "stage12_update_controller"
#%THISID%
...
mo.pointWorldToCam = pointWorldToCam
...
def getArrowTipCoord(html_text):
	...
	#determine if bubble needs to be wrapped (i.e. too big)
	label.setWordWrap(False)
	overlay_width = cmds.getAttr('controller.overlay_width')
...
if (style == "speech" and r_to == "UI code or scene object / component"):
	arrow_tip = getArrowTipCoord(text)
...
label=overlay.setAsLabel(html_text)
