...
def bt_SpeedCutUI():
    ...
    bt_SpeedCutWin = mc.window("bt_SpeedCutWin",title = "speedCut 1.69", mxb = False, s = 1 ,bgc = [0.2, 0.2, 0.2  ])
    mc.tabLayout(tv = 0)
    ...
    mc.text(l ='',h=20)
    mc.iconTextButton('preBevelButton',  w=40,style='textOnly', l= "Bevel Manager", rpt = True, c = 'bt_SpeedCutBevelMangerSetup()', bgc = [0.14, 0.14, 0.14] )
    mc.separator( height=15, style='none' )
    mc.setParent( '..' )
    mc.separator( height=15, style='none' )
    mc.showWindow(bt_SpeedCutWin)
    mc.window("bt_SpeedCutWin",e=True,w = 320,h = 1250)
    mc.commandEcho( state=False )
    mc.scriptEditorInfo(suppressWarnings=0,suppressInfo=0,se=0)
    checkBaseMeshList()
    allowedAreas = ['right', 'left']
    mc.dockControl('speedCutDock', l='speedCut 1.69', area='right', content = 'bt_SpeedCutWin',  allowedArea= allowedAreas, fcc = 'floatSCUIsize()')
    #start with floating UI
    #mc.dockControl('speedCutDock', l='speedCut 1.69', area='right', content = 'bt_SpeedCutWin',  allowedArea= allowedAreas, fcc = 'floatSCUIsize()',fl=1)
...
def killPreview():
    ...
    #if mc.window("bt_SpeedCutBevelManger", exists = True):
    #    mc.deleteUI("bt_SpeedCutBevelManger")
...
def bt_SpeedCutBevelMangerSetup():
    beseMesh = mc.optionMenu('baseMeshMenu', query = True, v = True)
    if len(beseMesh)>0:
        bt_SpeedCutBevelMangerUI()
        bakeCutter('all')
...
def bt_SpeedCutBevelMangerUI():
    if mc.window("bt_SpeedCutBevelManger", exists = True):
        mc.deleteUI("bt_SpeedCutBevelManger")
    bt_SpeedCutBevelManger = mc.window("bt_SpeedCutBevelManger",title = "speedCut Bevel Manger",w = 400,h = 380, mxb = False, s = 1, bgc = [0.2, 0.2, 0.2  ])
    mc.columnLayout()
    ...
    mc.text(l ='')
    mc.separator( height=15, style='none' )
    mc.showWindow(bt_SpeedCutBevelManger)
    obNum = mc.scriptJob ( p = 'bt_SpeedCutBevelManger', event = ["SelectionChanged", reselctList])
##################################################################################################################
