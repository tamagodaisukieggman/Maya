...
cmds.parent('left_knee_POS_LOC_GRP', 'left_thigh_ROT_LOC')
cmds.parent('left_ankle_POS_LOC_GRP', 'left_thigh_ROT_LOC')
cmds.parent('left_foot_POS_LOC_GRP', 'left_thigh_ROT_LOC')
...
# cmds.matchTransform(left_arm_loc, left_elbow_loc, pos=False, rot=True, scl=False)


# IK
# ik_joints = ['ik_'+j for j in cmds.ls(left_arm, dag=True, type='joint')]
# dups = cmds.duplicate(left_arm, rc=True, f=True)
# consts = [c for c in dups if 'Constraint' in c]
# cmds.delete(consts)
# dups = [d for d in dups if not d in consts]
# for d, ij in zip(dups, ik_joints):
#     cmds.rename(d.split('|')[-1], ij)

# ikh = cmds.ikHandle(sj=ik_joints[0], ee=ik_joints[-1], solver='ikRPsolver')


# Match
# vertices = cmds.ls(os=True, fl=True)
# for vtx, ctrl in zip(vertices, left_arm_pos_locs):
#     v_t = cmds.xform(vtx, q=True, t=True, ws=True)
#     cmds.xform(ctrl, t=v_t, ws=True, a=True)
