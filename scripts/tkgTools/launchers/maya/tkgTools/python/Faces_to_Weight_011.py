...
Input the top Joints, middle or right side only, if lelf side joints are there, they will be skipped

created by Truong Cg Artist
'''
...
# Top non symmetrical joints
topNonSymJnts = []
topNonSymJntsTF = "top_non_symmetry_joints_text_field"
nonSymJointsLists = []
flatNonSymJointsList = []
...
def createInfoLoc():
    if not cmds.objExists("infoLoc_FTW"):
        ...
        cmds.addAttr(infoLoc, longName='topNonSymJnts', dt='string')
...
def checkExistingVariables():
    ...
    if cmds.objExists("infoLoc_FTW"):
        ...
        try:
            topNonSymJnts = convertStrListIntoList(cmds.getAttr("infoLoc_FTW.topNonSymJnts"))
        except:
            print(".topNonSymJnts is not available")
...
def checkTypeModels(models):
    for model in models:
        shapeNode = cmds.listRelatives(model, shapes=True)[0]
        print(shapeNode)
        nodeType = cmds.nodeType(shapeNode)
        if not nodeType == "mesh":
            raise Exception("Please input meshes only, not other type")        

def checkExistingSkincluster(models):
    for model in models:
        modelHist = cmds.listHistory(model, pdo=True)
        skinCluster = cmds.ls(modelHist, type="skinCluster")
        if skinCluster:
            raise Exception("Existing skin cluster found. Please unbind the models first") 
...
def prepTheTempModel(models):
    ...
    if models:    
        ...
        cmds.parent(dupModels, world=True)
...
def checkDuplicatedNames(jntList):
    jntListSet = set(jntList)
    if len(jntList) != len(jntListSet):
        raise Exception("You're having duplicated joint names")

# filter out joints in other lists, use for Neck Joints & Torso Joints
# jointLists = torso joint lists
# otherJointLists = limb joint lists
def filterOutMatchingJoints(jointLists, otherJointLists):
    clearJointLists=[]
    for jointList in jointLists:
        clearJointList = []
        for jnt in jointList:
            # merge otherJointLists for easy checking            
            mergedOtherJointLists = []
            for i in otherJointLists:
                mergedOtherJointLists.extend(i)
            if jnt not in mergedOtherJointLists:
...
def listChildrenJoints(topJointName):
    jointList=[]
    jointList = cmds.listRelatives(topJointName, children=1, allDescendents=1)
    checkDuplicatedNames(jointList)

    if jointList:
        # need to add Root joint as well
        jointList.append(topJointName)
    else:
        jointList = [topJointName]
    return jointList

# Sort joints in jointsList by number of parents & number of childrens
def sortCondition(jnt):
    parentPath = cmds.listRelatives(jnt, parent=1, fullPath=1)
    if parentPath:
        parentNum = parentPath[0].count('|')
    else:
        parentNum = 0
    # when joints have the same number of parent (same hierarchy), compare number of children: more children go first
...
# Sort joints by group of continuous joints, break when parent joints have multiple branches
def findAllParents(jnt):
    parents = cmds.ls(jnt, long=True)[0].split('|')[1:-1]
    parents.reverse()
    return parents

def findAllParentsInJointList(jnt, jointList):
    parentsInJointList = []
    parents = findAllParents(jnt)
    if parents:
        for parent in parents:
            if parent in jointList:
                parentsInJointList.append(parent)
    print(parentsInJointList)
    return parentsInJointList

def trimListWhenBranchingOut(parentJntList):
    continuousList = []
    if parentJntList:
        for parentJnt in parentJntList:
            if len(cmds.listRelatives(parentJnt, children=1)) == 1:
                continuousList.append(parentJnt)
            else:
                break
    return continuousList

def checkIfListIsRepeated(needCheckList, allContinousJointList):
    for list in allContinousJointList:
        if list != needCheckList:
            if set(needCheckList).issubset(list):
                return True
    return False

def allContinousJointList(jointList):
    allContinousJointList=[]
    
    for jnt in jointList:
        continousJointList = [jnt]
        parentJointList = findAllParentsInJointList(jnt, jointList)
        if parentJointList:
            parentJointList = trimListWhenBranchingOut(parentJointList)
            if parentJointList:
                continousJointList.extend(parentJointList)
        allContinousJointList.append(continousJointList)

    noRepeatedList=[]
    for checkList in allContinousJointList:
        if not checkIfListIsRepeated(checkList, allContinousJointList):
            noRepeatedList.append(checkList)
...
def createContinuousJointList(jointList):
    flatList = []
    allConJointList = allContinousJointList(jointList)
    flatList = flatoutList(flatList, allConJointList)
    return flatList

# create joint lists
def createJointLists(topJointsList, leftSide):
    jointLists = []
    if topJointsList:
        for topJnt in topJointsList:
            if cmds.objExists(topJnt) and cmds.objectType(topJnt)=="joint":
                ...
                jointList = createContinuousJointList(jointList)
...
def createDeformJointsListFTW():
    ...
    # Arm joint lists
    if topArmJnts:
        armJointsLists = createJointLists(topArmJnts, False)
        print(str(armJointsLists)+" are arm joint lists")

    # Leg joint lists
    if topLegJnts:
        legJointsLists = createJointLists(topLegJnts, False)
        print(str(legJointsLists)+" are leg joint lists")

    # Tail joint lists
    if topTailJnts:
        tailJointsLists = createJointLists(topTailJnts, False)
        print(str(tailJointsLists)+" are tail joint lists")

    # Non symmetrical joint lists
    if topNonSymJnts:
        nonSymJointsLists = createJointLists(topNonSymJnts, True)
        print(str(nonSymJointsLists)+" are non symmetrical joint lists")
        flatNonSymJointsList = flatoutList([], nonSymJointsLists)

    # Facial joint lists
    if headJnts:
        facialJointsLists = createJointLists(headJnts, False)
        print(str(facialJointsLists)+" are facial joint lists")

    # Neck joint lists
    if topNeckJnts:
        if facialJointsLists:
            neckJointsLists = createJointLists(topNeckJnts, False)
    ...
    if rootJnts:
        torsoJointsLists = createJointLists(rootJnts, False)
        if topArmJnts:
        ...
        if topNonSymJnts:
            torsoJointsLists = filterOutMatchingJoints(torsoJointsLists, nonSymJointsLists)
    ...
    print("deformJntList here" +str(deformJointsListFTW))
   
    if topNonSymJnts:
        # skip duplicated items between deformJointsListFTW & nonSymJointsLists before adding to the deformJointsListFTW
        deformJointsListFTWTemp = deformJointsListFTW
        deformJointsListFTW = []
        for jntName in deformJointsListFTWTemp:
            if jntName not in flatNonSymJointsList:
                 deformJointsListFTW.append(jntName)
        deformJointsListFTW = flatoutList(deformJointsListFTW, nonSymJointsLists) 
...
def findCurrentFTWJoint(jointList):
    ...
    if cmds.ls(sl=1):
        if findCurrentFTWJointBasedOnSelection(jointList, cmds.ls(sl=1)):
            # this one create trouble, auto detect current joint should work just fine
            pass
...
def toggleIsolation():
    ...
    currentStage = cmds.isolateSelect(panel, state=1, q=1)
    print(currentStage)
    if currentStage:
        disableIsolateItems()
    else:
        if allJointsList and tempModel:
            isolateItems(allJointsList, tempModel)
        else:
            raise Exception("Variables are missing, please run Prep again")
...
def bindSkinFTW():
    ...
    # if there are non symmetrical joints lists, skip, then food them again after mirror the skin weight

    tempModelSkinCluster = cmds.skinCluster(allJointsList, tempModel, sm=0, tsb=True)[0]
    for jointName in deformJointsListFTW:
        if flatNonSymJointsList and jointName in flatNonSymJointsList:
            break
    ...
    # if there are non symmetrical joints lists, food them again after mirror the skin weight
    if flatNonSymJointsList:
        for nonSymJnt in flatNonSymJointsList:
            if cmds.objExists(nonSymJnt+"_FTWSet"):
                facesList = cmds.sets(nonSymJnt+"_FTWSet", q=True)
                if facesList:
                    verticesList = facesToVertices(facesList)
                    if verticesList:
                        floodSkinByJointName(tempModelSkinCluster, verticesList, nonSymJnt)
    ...
    for model in models:
        modelSkinCluster = cmds.skinCluster(allJointsList, model, sm=0, tsb=True)[0]       
...
def chooseModels():
    ...
    checkExistingSkincluster(models)
...
def chooseTopNonSymJoints():
    global topNonSymJnts
    topNonSymJnts = cmds.ls(sl=1)
    # start creating info locator when there is input
    createInfoLoc()
    load_selected_as(topNonSymJnts, topNonSymJntsTF)
    if cmds.objExists("infoLoc_FTW"):
        cmds.setAttr("infoLoc_FTW.topNonSymJnts", topNonSymJnts, type="string")
    else:
        raise Exception("infoLoc_FTW is missing")
...
def create_UI(winID):
    ...
    selectionTypeRow = cmds.rowLayout(numberOfColumns=3, p=columnMain)
    cmds.iconTextButton(label="Toggle Joint Selection", p=selectionTypeRow, width=toolWindowWidth*1/4, i='kinJoint.png', command='toggleJointSelection()')
    jointSizeFloatSlider = cmds.floatSlider(min=0.01, max=10, value=2, step=0.01, width=toolWindowWidth*2/4, dc='manualAdjustJointSize()')
    cmds.iconTextButton(label="Toggle Isolation", p= selectionTypeRow, width=toolWindowWidth*1/4, i='polyPrelight.png', command='toggleIsolation()')
    ...
    # non-symmetrical top joints
    loadNonSymJntsRow = cmds.rowLayout(numberOfColumns=2, p=inputTab)
    tf_topNonSymJntsItems = cmds.textField(topNonSymJntsTF, w=toolWindowWidth*0.7, h=20, p=loadNonSymJntsRow, ed=0)
    if topNonSymJnts:
        cmds.textField(tf_topNonSymJntsItems, e=True, tx=str(topNonSymJnts))
    cmds.button(label='Top Non-Sym Joint(s)', width=toolWindowWidth*0.3, align='right', p=loadNonSymJntsRow, command='chooseTopNonSymJoints()')
