...
from decoration import keep_selections
...
class ProgressWindowBlock(object):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        print('[ {} ] : End : Calculation time : {}'.format(self.title, calc_time))
...
def add_influences(nodes, influences):
    ...
    for node in nodes:
        skinclusters = list_related_skinClusters(node)
        if not skinclusters:
            cmds.warning('[ Skin add influences ] : {} : skincluster not found.'.format(node))
            continue
...
def remove_unused_influences(skincluster):
    ...
    if remove_influences:
        ...
        print('[ Remove Unused Influences ] : {} : {}'.format(skincluster, remove_influences))
...
def get_skinCluster_weights(object_name, **kwargs):
    ...
    if not cmds.objExists(object_name):
        cmds.warning('{} is not found.'.format(object_name))
        return

    skinClusters = list_related_skinClusters(object_name)
    if not skinClusters:
        cmds.warning('Related skinCluster not found : {}'.format(object_name))
        return
    ...
    try:
        ...
        cmds.warning(str(e))
        cmds.warning('Can not get components : {}'.format(object_name))
        return
...
def set_skinCluster_weights(object_name, weights, infls, comps=None, **kwargs):
    ...
    num_infls = infls.length()
    num_comps = int(num_weights / num_infls)
    ...
    if uncontain_infls:
        cmds.warning('{} is not in \'{}\' influences.'.format(uncontain_infls, skinCluster))
        return
...
def import_skinCluster_weights(object_name, file_path, method='index', force_bind=False, **kwargs):
    ...
    if method not in ['index', 'closestpoint', 'closestcomponent', 'uvspace']:
        cmds.warning('{} is Invalid import method.'.format(method))
        return False
    ...
    if not_found_influences:
        cmds.warning('{} is not fuond.'.format(not_found_influences))
        return False
    ...
    if method == 'index':
        if num_components != int(read_data.get('numComponents', 0)):
            cmds.warning('{} and {} components count missmatch.'.format(object_name, file_path))
            return False
    else:
...
def read_weights_data(file_path):
    ...
    if not os.path.isfile(file_path):
        cmds.warning('{} is not found.'.format(file_path))
        return {}
...
def select_geometry_from_data(file_path):
    ...
    if not cmds.objExists(geometry):
        cmds.warning('{} is not found.'.format(geometry))
        return
...
def select_influences_from_data(file_path):
    ...
    if not_found_influences:
        cmds.warning('{} is not found.'.format(not_found_influences))
        return
...
def bind_and_copy(src, dst, src_components=None, dst_components=None, bind=True, method='closestpoint'):
    ...
    if not src_clst:
        cmds.warning('skinCluster not found : {}'.format(src))
        return
    src_clst = src_clst[0]
    ...
    if not bind and not dst_clst:
        cmds.warning('skinCluster not found : {}'.format(dst))
        return
    ...
    if method == 'index':
        if get_component_count(['{}.cp[*]'.format(src)]) != get_component_count(['{}.cp[*]'.format(dst)]):
            cmds.warning('{} and {} components count missmatch.'.format(src, dst))
            return
        ...
        if src_shape_type != 'mesh' or dst_shape_type != 'mesh':
            cmds.warning('Import method \'uvspace\' is mesh-only method.')
            return
