...
class SkinWeightGUI(object):
    ...
    def on_import_weight_btn(self, *args):
        ...
        if not sels:
            cmds.warning(u'ジオメトリを選択して下さい。')
            return

        file_path = self.get_import_weight_file()
        if not os.path.isfile(file_path):
            cmds.warning(u'ウェイトファイルを選択して下さい。')
            return
        ...
        with command.WaitCursorBlock():
            for node in sels:
                ...
                if ret:
                    print('[ Import Weights ] : {} : {}'.format(node, file_path))
    ...
    def on_import_weights_btn(self, *args):
        ...
        with command.ProgressWindowBlock(title='Import Weights', maxValue=len(sels)) as prg:
            ...
            for node in sels:
                ...
                if not os.path.isfile(file_path):
                    cmds.warning('[ Import Weights Skip ] : {} : {} is not found'.format(node, file_path))
                    continue
    ...
    def on_export_weight_btn(self, *args):
        ...
        if not skinclusters:
            cmds.warning(u'スキニングされてたジオメトリを選択して下さい。')
            return
        ...
        with command.WaitCursorBlock():
            ...
            print('[ Export Weights ] : {} : {}'.format(node, export_file))
    ...
    def on_select_influences_from_weight_file(self, *args):
        ...
        file_path = self.get_import_weight_file()
        if not os.path.isfile(file_path):
            cmds.warning(u'ウェイトファイルが見つかりません。')
            return
    ...
    def on_bindskin_btn(self, *args):
        ...
        if not valid_nodes:
            cmds.warning(u'バインドできるジオメトリを選択して下さい。')
            return

        influences = cmds.ls(sl=True, et='joint')
        if not influences:
            cmds.warning(u'インフルエンスを選択して下さい。')
            return
    ...
    def on_unbindskin_btn(self, *args):
        ...
        if not valid_nodes:
            cmds.warning(u'skinClusterが接続されているジオメトリを選択して下さい。')
            return

        for node in valid_nodes:
            cmds.skinCluster(node, e=True, unbind=True)
            print('[ Unbind Skin ] : {}'.format(node))
    ...
    def on_add_influences_btn(self, *args):
        ...
        nodes = cmds.ls(sl=True, et='transform')
        if not nodes:
            cmds.warning(u'インフルエンスを追加するジオメトリを選択して下さい。')
            return
    ...
    def on_bind_and_copy_btn(self, *args):
        ...
        if len(sels) < 2:
            cmds.warning(u'ジオメトリを２つ選択して下さい。')
            return
        ...
        for src_geo, dst_geo in zip(src_geos, dst_geos):
            print('[ Copy Weights ] : {} to {}'.format(src_geo, dst_geo))
