...
class Spine(nmModule.RigModule, nmSpline.Spline):
    def __init__(self,
                 side=None,
                 part=None,
                 guide_list=None,
        ...
        spine_util = nmSpine.Spine(side='Cn',
                         part='spine',
    ...
    def control_rig(self):
        ...
        # build fk controls
        fk_chain = nmChain.Chain(prefix=self.side,
                                 suffix='ctrl_JNT',
                                 name=self.part)
        ...
        for i, jnt in enumerate(fk_chain.joints[:-1]):
            ...
            if i == 0:
                ...
                shape = 'halfCylinder'
            fk_ctrl = nmCtrl.Control(parent=par,
                                     shape=shape,
                                     prefix=self.side,
                                     suffix='CTRL',
                                     name=ctrl_name,
    ...
    def output_rig(self):
        ...
        if self.mid_ctrl:
            blend = nmAttr.Attribute(node=self.mid_ctrl.ctrl, type='double',
    ...
    def skeleton(self):
        spine_chain = nmChain.Chain(transform_list=self.spline_joints,
                                    prefix=self.side,
                                    suffix='JNT',
                                    name=self.part)
