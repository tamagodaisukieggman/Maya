            ...
            if copytype == 1:
                ...
                for right, left in zip(right_nodes, left_nodes):
                    if l_to_r:
                        ...
                        cls._reverse(right, node_axis[right], animtype, copytype=copytype, time=current, is_behavior=node_behavior[right])
                    else:
                        cls._copy_animation(right, left, animtype=animtype, copytype=copytype, time=current)
                        cls._reverse(left, node_axis[left], animtype, copytype=copytype, time=current, is_behavior=node_behavior[left])

            # reverse
            else:
                # センターノードの反転
                for node in center_nodes:
                    cls._reverse(node, node_axis[node], animtype, time=current)

                # 右、左のノードの反転
                for right, left in zip(right_nodes, left_nodes):
                    cls._copy_animation(right, left, animtype=animtype, copytype=copytype, time=current)
                    if not node_behavior[right]:
                        cls._reverse(right, node_axis[right], animtype, copytype=copytype, time=current)
                        cls._reverse(left, node_axis[left], animtype, copytype=copytype, time=current)
                    else:
...
class ReverseMotion(object):
    ...
    def _reset_settings(self, *args):
        ...
        cmds.textField(self._left_id_control, e=True, tx='L_')
        cmds.textField(self._right_id_control, e=True, tx='R_')
