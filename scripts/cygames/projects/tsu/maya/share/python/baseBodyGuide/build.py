...
def buildLeftClavicle(info={}, spEnd=''):
    ...
    if clList:
        ...
        pm.select(d=True)
        mergerotation.merge_rotation(clList[:-1], 'rotate', 1)
        clEnd = clList[-2]
...
def buildLeftArm(info, clEnd=''):
    ...
    # -- parent
    wrt.setParent(elb)
    mergerotation.merge_rotation([shd, elb, wrt], 'rotate', 1)
...
def buildLeftInHand(info={}, wtEnd=''):
    ...
    mergerotation.merge_rotation(endList, 'rotate', 1)
    print (log)
    return endList
...
def buildLeftFinger(info={}, fid=0, ihEnd=''):
    ...
    if fList: 
        ...
        fEnd.r.set(0,0,0)
        fEnd.jo.set(0,0,0)
        mergerotation.merge_rotation(fList, 'rotate', 1)
    else:
...
def buildLeftLeg(info={}):
    ...
    pm.delete(pm.aimConstraint(kne, hip,
                              sk = 'x',
                               w = 1.0,
                               o = v[0],
                             aim = v[1],
    ...
    # -- parent
    hip.setParent(plv)
    mergerotation.merge_rotation([hip], 'rotate', 1)
    # -- L knee
    kne.setParent(hip)
    kne.jo.set(0,0,0)
    mergerotation.merge_rotation([kne], 'rotate', 1)
    # -- L ankle
    akl.setParent(kne)
    akl.r.set(0,0,0)
    akl.jo.set(0,0,0)
...
def fixThumbRotation():
    ...
    pm.delete(pm.aimConstraint(lt1, lt0, o=(0,0,0),
                                       ...
                                       wuo='_00d',
                                        sk=('x', 'y')))
    lt1.setParent(lt0)   
    mergerotation.merge_rotation(['_201'], 'rotate', 1)
    ...
    pm.delete(pm.aimConstraint(rt1, rt0, o=(0,0,0),
                                       ...
                                       wuo='_009',
                                        sk=('x', 'y')))
    rt1.setParent(rt0)   
    mergerotation.merge_rotation(['_101'], 'rotate', 1)
