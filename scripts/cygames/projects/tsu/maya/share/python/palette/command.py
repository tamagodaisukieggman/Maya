...
# ---- Script TypeがInternalかチェック
def _get_script_node():
    f = lambda n: True if cmds.getAttr("{}.st".format(n)) != 3 and cmds.getAttr("{}.st".format(n)) != 6 else False
    script_nodes = [node for node in cmds.ls(typ="script") if f(node)]
    return script_nodes
...
def setDefaultUnitEnv():
    log = '------------------- Set Unit Log ---------------------------------\n'
    log += setGrid(200.0, 50.0, 5)
    log += setCam(50)
    log += setJointSize(1.0)
    log += setSmoothWireframe(8)
    log += setSceneUnit('cm', 'deg', '60fps')
    log += setTimeRange(0, 10)
