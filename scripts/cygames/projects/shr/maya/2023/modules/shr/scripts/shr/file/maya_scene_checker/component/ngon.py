...
def check_ngon_api2(data_type_category="", mesh_fn="", dag_path="", fullPath='', _result_datas:scene_data.ResultDatas=None):
    mesh_fn = None
...
def check(data_type_category:str='', scene_path:str='', check_targets:list=[])->scene_data.ResultDatas:
    ...
    for x in range(selList.length()):
        dag_path = selList.getDagPath(x)
        fullPath = dag_path.fullPathName()
        check_ngon_api2(data_type_category, dag_path, fullPath, _result_datas)
