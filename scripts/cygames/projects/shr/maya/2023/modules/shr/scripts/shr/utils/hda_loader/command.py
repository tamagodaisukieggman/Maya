...
from . import NAME
from . import INIT_FILE
from . import YAML_FILE_NAME



def load_config(config_name:str = '', config_file_path:str = ''):
    if not config_file_path:
        config_file_path:Path = INIT_FILE / YAML_FILE_NAME

    if not config_file_path.exists():
        return

    with open(str(config_file_path).replace(os.sep, '/'), encoding='utf-8') as f:
