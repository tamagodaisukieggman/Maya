...
class SceneData:
    ...
    def set_asset_type(self):
        ...
        _metahuman_node_name:str = settings.load_config('METAHUMAN_NODE_NAME')
        _metahuman_node:list = cmds.ls(type=_metahuman_node_name)

        if _metahuman_node:
            _data_type_category = 'METAHUMAN'
        else:
...
class NodeDatas:
    ...
    def get_meshes(self)->None:
        if not self.root_nodes:
            return
        for node in self.root_nodes:
            meshes:list = cmds.listRelatives(node, allDescendents=True, fullPath=True, type="mesh")
            if meshes:
                # 中間オブジェクトを排除
                meshes = [x for x in meshes if not cmds.getAttr("{}.intermediateObject".format(x))]
