...
_grp_node_suffix = "_grp"

def _check_name_hierarchy_second_chara(node):
    if not node:
        return []
    ...
    if _second_childern:
        for _cld in _second_childern:
            ...
            _name = _name_split[-1]
            if _parent == _model_group_name:
                if cmds.nodeType(_cld) != "transform":
                    errors.append(_cld)
                    # errors.append(5)
                _cld_nodes = cmds.listRelatives(_cld, children=True, fullPath=True)
                if not _cld_nodes:
                    continue
                if cmds.nodeType(_cld_nodes[0]) != "mesh" and not _name.endswith(_grp_node_suffix):
                    errors.append(_cld)
                    # errors.append(6)
