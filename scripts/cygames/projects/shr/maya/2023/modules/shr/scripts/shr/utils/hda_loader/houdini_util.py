...
class HoudiniEngineMaya:
    ...
    def _set_trust_path(self) -> None:
        ...
        paths:list = cmds.optionVar(q=_SafeModeAllowedlistPaths)
        new_paths:list = []
        print(f'\n{"Trust directorys ":-<100}')
        if paths:
            for path in paths:
                if path == houdini_engine_path and path not in new_paths:
                    new_paths.append(path)
                elif 'houdini_engine' not in path:
                    new_paths.append(path)
            print(f'{"":-<100}')
        if houdini_engine_path not in new_paths:
