...
class ue_client:
    ...
    def get_project_content_directory(cls) -> str:
        """get_project_content_directoryで取得的ルcontentの絶対パスを取得
        ...
        path = result["output"][0]["output"].split("'")[1]
        return path

    @classmethod
    def get_engine_absolute_path(cls, asset_path):
        asset_path = asset_path.replace("\\", "/")
        asset_path = asset_path.replace("/Game/", cls.get_project_content_directory())
        return asset_path

    @classmethod
    def get_engine_relative_path(cls, asset_path):
        asset_path = asset_path.replace("\\", "/")
        asset_path = asset_path.replace(cls.get_project_content_directory(), "/Game/")
...
class ue_gui_utils:
    ...
    @classmethod
    def _get_asset_data(cls, id: str, ws_type: str):  # -> Optional[AssetData]:
        """assetidの取得
        ...
        asset_data = get_ws_name.from_asset_name(id, ws_type)
    ...
    def get_anim_fbx_path(cls, file_name: str):
        ...
        asset_data = cls._get_asset_data(id, "Animations")
        fbx_path = str(asset_data.workspace)
    ...
    @classmethod
    def get_sk_uasset_path(cls, fbx_path: str) -> str:
        ...
        asset_data = cls._get_asset_data(id, "Character")
        ...
        # 相対pathをGamePathに変換
        asset_path = ue_client.get_engine_relative_path(asset_path)
...
class ue_importer:
    ...
    def sk_uasset_import(
        ...
        for i, fbx_path in enumerate(export_paths):
            import_path = ue_gui_utils.get_sk_uasset_path(fbx_path)  # + fbx_file_name
