                        ...
                        if sumPath.count('.json') > 0 or sumPath.count('.json') > 0:
                            ...
                            if i == 0:
                                ...
                                for pFol in poseFolders:
                                    ...
                                    # print poses
                                    for addPose in poses:
                                        if addPose.count('.png') > 0:
                                            pngpath = addPose.replace('\\', '/')
                                            # pngImg.append(pngpath)
                                            # pngpath = pngImg[0]
                                            Img = pngpath.split('/')
                                            lbl = Img[-1].split('.')
                                            pngpathGp = pngpath.replace('/', '_')
                                            Img.remove(Img[-1])
                                            Img = '/'.join(Img)
                                            jpath = pngpath.split('.')
                                            Jpath = jpath[0] + '.json'
                                            ...
                                            poseName = dir[-2]
                                            if tabName == poseName:
                                                mc.iconTextButton(pngpathGp, parent=rowsB, width=imgSize, h=imgSize, label=lbl[0], ann=lbl[0], fla=True, style='iconAndTextVertical', sic=True,  image1=pngpath, c=partial(iJsn.importJSON, Jpath))
                                                popup = mc.popupMenu(b=0)
                                                expPath = pPathA.replace(r'/', r'\\')
                                                expPath2 = r''.join(expPath)
                                                pngpath2 = pngpathGp.replace('.', '_')
                                ...
                                for pFol in poseFolders:
                                    ...
                                    for addPose in poses:
                                        if addPose.count('.png') > 0:
                                            ...
                                            poseName = dir[-2]
                                            if tabName == poseName:
                                                mc.iconTextButton(pngpathGp, parent=rowsC, width=imgSize, h=imgSize, label=lbl[0], ann=lbl[0], fla=True, style='iconAndTextVertical', sic=True,  image1=pngpath, c=partial(iJsn.importJSON, Jpath))
                                                popup = mc.popupMenu(b=0)
                                                expPath = pPathA.replace(r'/', r'\\')
                                                expPath2 = r''.join(expPath)
                                                pngpath2 = pngpathGp.replace('.', '_')
