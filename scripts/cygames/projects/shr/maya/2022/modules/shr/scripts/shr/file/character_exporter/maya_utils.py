...
class MayaUtil:
    ...
    @classmethod
    def disconnection_est3_constraint(cls, target_joints: list) -> None:
        """eST3transformConstraint関係のconnectionを切断

        Args:
            joints (list): コネクションを切断したいjointを与える
        """
        for joint in target_joints:
            # eSTのコンストレインのコネクションを切断
            connections = (
                cmds.listConnections(
                    joint, c=True, p=True, type="eST3transformConstraint"
                )
                or []
            )
