...
from shr.file.character_exporter.maya_utils import MayaUtil
...
class Exporter:
    ...
    def _model_prep(self) -> bool:
        ...
        try:
            ...
            # 不要なコンストレインを削除
            cmds.delete(cmds.ls("*Constraint?"))
            cmds.delete(cmds.ls("pairBlend*"))

            # eST関係のConstraintとコネクションを切る
            cmds.delete(cmds.ls(type="eST3rigConnector"))
            cmds.delete(cmds.ls(type="eST3transformConstraint"))
    ...
    def _animation_prep(self) -> bool:
        ...
        try:
            selected = cmds.ls(sl=True)
            target_joints = self._get_target_joints(selected)
            # all_joints = self._get_target_joints(selected)
            ...
            MayaUtil.disconnection_est3_constraint([root])
    ...
    def export(self, path="", file_name="") -> None:
        ...
        try:
            ...
            cmds.error("fbxの書き出しができませんでした。保存先が正しいかご確認ください。")
    ...
    # パーツごとの書き出しは廃止
    # def _export_body_parts(self, selected_joint: str, path: str, file_name: str):
    #     exported_fbx_paths = []
    #     for lp in self.export_target_grps:
    #         cmds.select([lp, selected_joint], r=True)
    #         grp_short_name = MayaUtil.get_short_name(lp)
    #         # pathからplyなどの識別部分をreplaceして整形
    #         path = path.replace("{}".format(file_name[:3]), "{}".format(lp[:3]))

    #         # export pathの整形
    #         exportpath = path + "/{}".format(self.get_fbx_prefix()) + grp_short_name

    #         os.makedirs(path, exist_ok=True)
    #         om.MGlobal.executeCommand('FBXExport -f "{0}.fbx" -s'.format(exportpath))
    #         print("exported >> %s.fbx" % exportpath.replace("/", "\\"))
    #         exported_fbx_paths.append(exportpath)
    #     return exported_fbx_paths
