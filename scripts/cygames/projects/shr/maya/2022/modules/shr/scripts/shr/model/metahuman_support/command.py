        ...
        if _transform:
            ...
            if _transform.endswith('_mesh'):
                ...
                if result.endswith('0'):
                    chenge_outliner_color(node=result)
...
def create_lodgroup():
    ...
    lod_mesh_group:str = cmds.createNode("lodGroup", name=_lod_group_name, skipSelect=True)
    ...
    # show_message('Create LOD Groups')
...
def get_geometory_root_node()->str:
    ...
    if not _meshes:
        lod_root_node:list = cmds.ls(load_config('lod_group_name'), type='lodGroup', long=True)
        if lod_root_node:
            lod_root_node = lod_root_node[0]
        else:
            lod_root_node = ''
            # print(lod_root_node, " -----------------")
            # if lod_root_node and cmds.listRelatives(lod_root_node, parent=True):
            #     lod_root_node = cmds.parent(lod_root_node, world=True)[0]
            # else:
            #     lod_root_node = ''
...
class MetahumanJointReConnections:
    ...
    def connect_attribute(self):
        ...
        for joint, attributes in self.joints_connections.items():
            # print(joint, attributes , " ---")
            # if not cmds.objExists(joint):
            #     continue
            for source_attirubute, target_attribute in attributes:
                # node:str = source_attirubute.split('.')[0]

                if not cmds.objExists(source_attirubute) and not cmds.objExists(target_attribute):
                    continue
                # print(source_attirubute, cmds.objExists(source_attirubute))
                # print(target_attribute, cmds.objExists(target_attribute))
                # try:
...
def set_current_character_path():
...
def get_node_from_selection(is_single:bool=True):
    result_node:str = ''
    ...
    if is_single:
        result_node = [selections[0]]
    else:
        result_node = selections
    return result_node
...
def import_metahuman_weight_to_selections(weight_type:str='Metahuman'):
    ...
    with gui_util.ProgressDialog(title='Import Skin Weight', maxValue=_length) as prg:
        ...
        for i, mesh in enumerate(meshes):
            ...
            mesh_weight_file = file_path_dict.get(mesh_short_name)
            if not mesh_weight_file:
                weight_file_name = mesh_short_name.rsplit('Shape', 1)[0]
                weight_file_name = f'{weight_file_name}_meshShape'
                mesh_weight_file = file_path_dict.get(weight_file_name)
                joints = joints_dict.get(weight_file_name)
            else:
            ...
            print(mesh_weight_file)


def delete_history_reconneciotns_from_selection(is_reset_transform:bool=False):
    nodes:str = get_node_from_selection(is_single=False)
    if not nodes:
        cmds.warning('select mesh node')
        return
    for node in nodes:
        delete_history_reconneciotns(node=node, is_reset_transform=is_reset_transform)
    cmds.select(nodes, replace=True)

def delete_history_reconneciotns(node:str='', is_reset_transform:bool=False):
    mesh:str = ''
    blend_shape:str = ''
    skin_cluster:str = ''
    joints:list = []
    ...
    if not node or not cmds.objExists(node):
        cmds.warning(f'not found: [ {node} ]')
        return

    meshes:list = cmds.listRelatives(node, allDescendents=True, type='mesh', fullPath=True)
    if not meshes:
        return
    meshes:list = [x for x in meshes if not cmds.getAttr(f"{x}.intermediateObject")]
    if not meshes:
    ...
    if not blend_shape:
        cmds.warning(f'no blendshape: [{mesh_short_name}]')
        return
    ...
    # ブレンドシェイプの元の名前を変えると接続できなくなるので注意
    blend_shape = cmds.blendShape(mesh, name=blend_shape)
    if not blend_shape:
        return
    blend_shape = blend_shape[0]
    skin_cluster = create_skin_cluster(joints=joints, node=mesh)

    for i, (source_attirubute, target_attribute) in enumerate(zip(blend_shape_source_cone, blend_shape_target_cone)):
...
def _delete_history_reconnection():
    """ハードコードで成功した例を残している
    """
    mesh = 'head_lod0Shape'
    blend_shape = 'head_lod0_mesh_blendShapes'
    embed = 'rl4Embedded_nfc0010_rl'
    skin_cluster = 'head_lod0Shape_skinCluster'
    connect = []
    _conec = cmds.listConnections(blend_shape, c=1, p=1,d=0,s=1,type='embeddedNodeRL4')
    joints = cmds.skinCluster(skin_cluster, influence=True, q=True)
    conec = [x for x in cmds.listConnections(blend_shape, c=1, p=1, d=1,s=0,type='skinCluster')]
    target = conec[0::2]
    souece = conec[1::2]
    _target = _conec[0::2]
    _souece = _conec[1::2]
    ...
    #print(cmds.blendShape(blend_shape, q=True, t="")
    #for source_attirubute, target_attribute in zip(_souece, _target):
    #    print(source_attirubute, cmds.nodeType(source_attirubute))
    #    print(target_attribute, cmds.nodeType(target_attribute))

    cmds.delete(mesh, ch=1)
    #cmds.select(mesh, r=1)
    cmds.blendShape(mesh, n=blend_shape)
    ...
    for source_attirubute, target_attribute in zip(souece, target):
        print(source_attirubute, target_attribute)
        try:
            cmds.connectAttr(source_attirubute, target_attribute, f=True)
        except:pass
        #print(source_attirubute, cmds.nodeType(source_attirubute))
        #print(target_attribute, cmds.nodeType(target_attribute))

    for i, (source_attirubute, target_attribute) in enumerate(zip(_souece, _target)):
        target = target_attribute.split('.', 1)[-1]
        #cmds.aliasAttr head_tiltRight_D head_lod0_mesh_blendShapes.w[680];
        cmds.aliasAttr(f'{target}', f'{blend_shape}.w[{i}]')
        #print(source_attirubute, cmds.nodeType(source_attirubute))
        #print(target_attribute, cmds.nodeType(target_attribute))
        try:
...
def _connect_attribute(node):
    # skin_cluster = create_skin_cluster(joints=joints, node=long_name)
    cmds.connectAttr("head_lod0Shape.inMesh", "head_lod0Shape_skinCluster.outputGeometry[0]")
    cmds.connectAttr("head_lod0Shape_skinCluster.outputGeometry[0]", "head_lod0Shape.inMesh")


def connect_attribute(root_node:str='', connection_source_target:dict={}):
    if not root_node and connection_source_target:
        return

    for node, attributes in connection_source_target.items():
        if not cmds.objExists(node):
            continue
        for source_attirubute, target_attribute in attributes:
            # node:str = source_attirubute.split('.')[0]
            print(source_attirubute)
            print(target_attribute, " ------")
            if not cmds.objExists(source_attirubute) and not cmds.objExists(target_attribute):
                continue
            if cmds.nodeType(source_attirubute) == 'transferAttributes':
                continue
            # try:
            cmds.connectAttr(source_attirubute, target_attribute)
            # except Exception as e:
            #     print(e)


def un_bind_facial_part():
    scene_name, root_node = get_shenron_nodes()
    if not scene_name or not root_node:
        return

    is_done = un_bind(root_node=root_node)
...
def un_bind(root_node:str='')->bool:
    if not root_node or not cmds.objExists(root_node):
        return
    meshes = cmds.listRelatives(root_node, allDescendents=True, type='mesh', fullPath=True)
...
def _un_bind():
    ...
    metahuman_root_joint:str = metahuman_root_joint[0]
    geometory_node:str = geometory_node[0]
    meshes = cmds.listRelatives(geometory_node, allDescendents=True, type='mesh', fullPath=True)
...
def import_weight_test():
    _other_weight_path:Path = Path(load_config('Other_weight_Filepath'))
    scene_name, root_node, lod_gropus = check_nodes()
    print(scene_name, root_node, lod_gropus)
    if not scene_name or not root_node and not lod_gropus:
        return
    _file = Path(r"C:\cygames\shrdev\shr_art\resources\characters\player\pfc\pfc2000_jd\work\maya\easy_bind\__pfc2000__mesh__lod0__head_lod0_grp__head_lod0__head_lod0Shape.xml")
    # _file = Path(r"C:\cygames\shrdev\shr\tools\in\ext\maya\2023\modules\shr\scripts\shr\model\metahuman_support\metahuman_head_weights\__pfc2000__mesh__lod0__head_lod0_grp__head_lod0__head_lod0Shape.xml")
    _file = Path(r"C:\cygames\shrdev\shr_art\resources\characters\player\pfc\pfc2000_jd\work\maya\easy_bind\__pfc2000__mesh__lod7__head_lod7_grp__head_lod7__head_lod7Shape.xml")
    joints_dict:dict = {}
    file_path_dict:dict = {}
    print(_file.exists())
    if not _file.exists():
        return joints_dict, file_path_dict
    ...
    long_name = '|pfc2000|mesh|lod0|head_lod0_grp|head_lod0'
    long_name = '|nfc0015|mesh|head_lod0_grp|head_lod0'
    long_name = '|nfc0015|mesh|head_lod7_grp|head_lod7'

    # _length = len(lod_gropus)
    ...
    if not joints:
        cmds.warning('no joint weight file')
        return
    print("-------------------")
    print(len(joints))
    if not skin_cluster:
    ...
    cmds.skinCluster(skin_cluster, edit=True, forceNormalizeWeights=True)
    # restore_bindpose(joints=joints)
    print("end func")
...
class ImportSkinWeight:
    ...
    def apply_shenron_weights(self):
        ...
        # print(scene_name, root_node, lod_groups, "=============")
        if not scene_name or not root_node and not lod_groups:
            return
        # print(lod_groups, " -----")
        self.lod_groups = lod_groups
        joints_dict, file_path_dict = get_weight_file_jointsdict(weight_directory='Default')
