...
from wmutils import wmlog, wmpath
from workfile_manager import p4utils, config, cmds as wcmds
    ...
    def execute(self, presetname=None, inputfile=None, argfile=None):
        ...
        if 'is_custom_task' in args and args['is_custom_task']:
            ...
            if 'assetdict' in args:
                ...
                if len(buf) == 0:
                    # モーションビルダーからパブリッシュセレクションした場合はoutfileはfbxファイル名になっている。
                    outfile_from_motionbuilder = wmpath.WMPath(outfile).change_extension('fbx').as_posix()
                    buf = db.get_sharedasset_from_file(outfile_from_motionbuilder)
        ...
        if 'is_custom_task' not in args or not args['is_custom_task']:
            ...
            ext_maps = {'mb':'mayaBinary', 'ma':'mayaAscii'}
            m = re.search('[.]([^.]+)$', outfile)
            if not m:
        ...
        if args['publish_app'] == Application.Maya:
            ...
            subprocess.Popen('mayabatch -file %s -command "%s"' % (outfile, cmd.replace('"', '\\"')))
            ...
            if submit_server:
                db.publish_share_refs(asset, assetdict['asset_id'], outfile, refs_on_share, args['user'] if 'user' in args else p4u.user, source_textures=source_textures)
            ...
            if 'from_publish_files' in args:
                ...
                wcmds.clean_up_local_tempdir(timedelta(days=1), subdir='local_publish_data')
                # ローカルパブリッシュ用一時保存場所の古いデータ（1日以上経過しているもの）を削除する

                #
                published_files: list[ReferenceInCommitTarget] = []
                backup_dir:str = wcmds.get_local_tempdir_for_now(makedirs_if_not_exist=True, subdir='local_publish_data')
                copied_file = shutil.copy2(outfile, backup_dir)
                logger.info('Copied: %s -> %s', outfile, copied_file)
                published_files.append(ReferenceInCommitTarget(outfile, copied_file))
        ...
        if args['commit_to_engine']:
            ...
            work_asset_info = CommitTargetWorkInfo(args['original_work_file'] if 'original_work_file' in args else None, copied_inputfile, references_in_workfile)
            share_asset_info = CommitTargetShareInfo(outfile, args['tags'], assetdict, published_files, thumb_share, fbx_filename=copied_fbx_file)
    ...
    if len(task.tracebacks) > 0:
        ...
        try:
            ...
            if channel is not None:
                notif.send_to_slack(message, channel, mentions=['takeuchi_kengo'], is_channel=True, channel_mention=False, header=(p4u.user, u'パブリッシュ時のエラー通知'))
            else:
