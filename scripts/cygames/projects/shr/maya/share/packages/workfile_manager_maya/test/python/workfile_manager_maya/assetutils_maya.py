...
def replace_to_cache_online(rfn:str|None=None, restore:bool=False, texture:bool=True, force_reload:bool=False) -> None:
    """
    Arguments:
        force_reload(bool):
            True: Referenceのload/unload状態に関わらず処理を行う
            False: Referenceがunload状態のものに対してのみ処理を行う
        restore(bool):
            True: Shareキャッシュファイルのパスが設定されているときは、元のShareパスに戻す
            False: Shareキャッシュファイルのパスが設定されている場合、必要に応じてローカルキャッシュを生成する（パスは変更しない）
    """
    if False:
    ...
    if rfn is None:
        ...
        filename = assetdbutils.normalize_path(cmds.referenceQuery(rfn, filename=True, wcn=True))
...
class WorkAssetMaya(assetvar.WorkAsset):
    def asset_specific_ui_properties(self, property_window, table) -> AssetSpecificProperties|None:
    ...
    def open(self, filename, args=None):
        ...
        cached_share_safe_open(filename, args=args)
    ...
    def exportself(self, filename, args=None):
        ...
        if 'skip_texture_check' not in args or not args['skip_texture_check']:
            if not uiutils_2.check_texture_path(proj_dir, self.dccutils().get_instance()):
...
class ShareAssetMaya(assetvar.ShareAsset):
    ...
    def open(self, filename, args=None):
        cached_share_safe_open(filename, args)
...
class MayaUtils(dccutils.DccUtils):
    ...
    def export_thumbnail(self, filename, ver):
        ...
        _jpg = os.path.join(wcmds.get_temp_dir(), basename)
    ...
    def get_filename(self) -> str:
        return assetdbutils.normalize_path(cmds.file(q=True, sn=True))

    def set_texture(self, texture_node_name, tex_path) -> None:
        buf = [x for x in texture_types if x['type']==cmds.objectType(texture_node_name)]
        assert len(buf) > 0
        _type = buf[0]
        
        if _type['type'] == 'file':
            cs = cmds.getAttr(texture_node_name + '.colorSpace')
            print('orginal color space: ', cs)

        cmds.setAttr(texture_node_name + '.'+_type['attr'], tex_path, type='string')

        if _type['type'] == 'file':
            cmds.setAttr(texture_node_name + '.colorSpace', cs, type='string')
            print('color space2: ', cmds.getAttr(texture_node_name + '.colorSpace'))

    def get_texture_node_names_and_paths(self) -> list[tuple[str, str]]:
        files = cmds.ls(type='file')
        results:list[tuple[str, str]] = zip(files, [assetdbutils.normalize_path(cmds.getAttr(x+'.fileTextureName')) for x in files])
        return results
...
def extract_embeded_scene_desc(filename):
    ...
    create_transform_sentence = b'XFRMCREA'
    buf = None
    buf_XFRMCREA = b''
    cnt = 0
    accum = b''

    max_bytes_to_read = 30000000    # 30MB
                                    # fileInfoがどこで終了するかを判定できていないため読み込みバイトに上限を設けている
                                    # 巨大なfileInfoが埋め込まれているケースでは調整が必要になり得る
                                    # 当初38.4KBという小さい値にしていたが、その場合AnimSchoolPickerが埋め込む大きなfileInfoに対応できなかった

    try:
        for i in range(int(max_bytes_to_read/64)): # TODO: 
            data = fhd.read(64)
            ...
            buf_XFRMCREA += data
            if (create_transform_sentence + b'\x00') in buf_XFRMCREA:
                break
        ...
        import traceback
        print(traceback.format_exc())
        message=u'シーンファイルにパイプラインメタ情報が存在しません。\n\n' + filename+ '\n'
        try:
            ui_dialogs.ErrorDialog(message=message).exec_()
        except:
            pass
        from workfile_manager import p4utils
        from wmutils import notification
        p4u = p4utils.P4Utils.get_instance()
        notification.send_to_slack(message + '\n\nfrom ' + p4u.user, 'takeuchi_kengo', header=(p4u.user, None))
        return None
...
def cached_share_safe_open(filename, args=None):
    try:
        if args is None:
            args = {}

        #if 'loadNoReferences' not in args:
        #    args['loadNoReferences'] = True
