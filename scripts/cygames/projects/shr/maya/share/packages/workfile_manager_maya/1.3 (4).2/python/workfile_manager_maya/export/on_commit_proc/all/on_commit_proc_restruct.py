...
class Plugin(OnCommitProcBase, object):
    ...
    def set_textures(self, p4u, files:list[FileToRegister]) -> None:
        """
            ...
            for f in nodes:
                ...
                for path in buf:
                    ...
                    if assetutils.is_non_cached_share_file(path):
                        ...
                        # 
                        # ↑
                        # DBにはここで取得したリビジョンを直接登録する（存在しているファイルのリビジョンを改めては確認しない）
                        # ので、ファイルさえ存在してシーンにパスをセット出来ればよい。
                        # revertして取得したリビジョンの状態を復元する必要はない。
    ...
    def execute(self, args) -> CommitProcessResult:
        ...
        #if 'submit_server' in args and args['submit_server']:
        self.set_textures(p4u, commit_process_result.files_to_register)
