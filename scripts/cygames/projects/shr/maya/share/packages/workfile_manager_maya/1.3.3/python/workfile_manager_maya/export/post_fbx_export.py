...
def create_thumbnail(thumb) -> str:
...
class Task:
    ...
    def execute(self, presetname=None, inputfile=None, argfile=None):
        ...
        if args['publish_app'] == Application.Maya:
            ...
            #cmd = 'python("import setup_callback_onsave.setup;setup_callback_onsave.setup.embed_scene_desc(None)");file -f -save;quit;'
            #subprocess.call('mayabatch -file %s -command "%s"' % (output_mb_filename, cmd.replace('"', '\\"')))
            cmds.file(output_mb_filename, o=True, f=True)
