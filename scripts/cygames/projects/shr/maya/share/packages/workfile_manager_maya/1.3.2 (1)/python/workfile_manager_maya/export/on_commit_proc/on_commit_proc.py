...
def execute(argfile, open_scene=True):
    ...
    for target_idx, target in enumerate(args['targets']):
        ...
        for _mod in args['procs']:
            ...
            pp = mod.Plugin()
            print('[ON_COMMIT_PROC] ', pp.getlabel())
            ...
            print('[DONE] ', pp.getlabel())

    print('files_to_submit: ', files_to_submit)
    ...
    if 'lazy_submit' in args and args['lazy_submit']:
        ...
        print('lazy_publish_cnts: ', args['lazy_publish_cnts'])
        ...
        if 'submit_server' in args and args['submit_server']:
            res_org = p4u.submitfiles(files_to_submit, desc=u'%s - Commited by %s' % (args['comment'], args['user']), submit_outside=True)
