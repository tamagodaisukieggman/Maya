...
def postfunc(presetname=None, inputfile=None, argfile=None):
    ...
    for maya_job in maya_jobs:
        ...
        if len(args['target_files']) > 1:
            # target_filesが複数ある場合、post_publish_files_mayaで異なるファイルがオープンされている状態でここにくるので、
            # このタイミングでシーンを改めてオープンする。
            try:
                assetutils_maya.cached_share_safe_open(maya_job['inputfile'])
