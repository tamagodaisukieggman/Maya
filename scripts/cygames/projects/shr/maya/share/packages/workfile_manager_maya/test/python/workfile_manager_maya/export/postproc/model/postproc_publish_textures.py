                ...
                for filepath_on_node in filepaths_on_node:
                    if filepath_on_node not in files_done:
                        ...
                        if submit_server:
                            try:
                                if rev is None:
                                    # パブリッシュ操作の過程で生成もしくは、置き換えられるテクスチャーの場合
                                    # 最新リビジョンを使用する
                                    p4u.p4_run_xxx('sync', '--parallel=threads=8', filepath_on_node)
                                else:    
                            ...
                            try:
                                p4u.p4_run_xxx('revert', '-a', filepath_on_node)
                            except:
                                pass
                    ...
                    if submit_server:
                        assetutils_maya.MayaUtils.get_instance().set_texture(fn, share_texpath)
