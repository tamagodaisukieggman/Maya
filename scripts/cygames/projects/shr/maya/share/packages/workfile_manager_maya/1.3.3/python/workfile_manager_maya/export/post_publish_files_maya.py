...
def postfunc(presetname=None, inputfile=None, argfile=None):
    ...
    from postproc_set_editor import post_process_operator, ui_preset, ui_main
    ...
    for original_work_file in args['target_files'][pid::maxproc]:
        ...
        try:
            cmds.file(original_work_file, o=True, f=True, iv=True)
        except Exception as e:
            print(e, flush=True)
        ...
        ppsets = dcc_cmds.list_dcc_sets_for_postproc(include_reference=False)

        if True or len(ppsets) == 0: # no sets found.
            if 'presets_if_no_sets' in args:
                presets = args['presets_if_no_sets']
                for preset_name in presets.keys():
                    
                    if len(ppsets) == 0:
                        print('Creating set from preset: ', preset_name, flush=True)
                        ui_preset.create_set_from_preset(presets, preset_name, dcc_cmds)
                    else:
                        dcc_set_name = dcc_cmds.get_name(ppsets[0])
                        print(f'Updating set:{dcc_set_name} with preset:{preset_name}', flush=True)
                        setobj = postproc_set_editor.Set(new=False, dcccmds=dcc_cmds, node_name=dcc_set_name)
                        ui_preset.setup_operators_for_set(setobj, presets, preset_name, dcc_cmds)
        ...
        if not saved:
            shutil.copyfile(original_work_file, outfile)
