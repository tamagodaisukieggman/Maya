...
class DccCmds(postproc_set_editor.DccCmdsBase):
    ...
    def open(self, filename):
        from workfile_manager_maya import assetutils_maya
        assetutils_maya.cached_share_safe_open(filename)
    ...
    def get_dcc_set_object(self, setname) -> pm.PyNode:
    ...
    def export_selection(self, filename, file_format):
        try:
            cmds.select('persp', add=True)
        except:
            print(traceback.format_exc())
            
        cmds.file(filename, f=True, es=True, type=file_format)   
