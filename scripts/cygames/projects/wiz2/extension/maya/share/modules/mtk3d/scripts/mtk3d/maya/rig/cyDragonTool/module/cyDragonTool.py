...
def ikToFkBake(ui, *args):
    ...
    leftIkArm = [Nmsp + 'footF_L_Ctrl', Nmsp + 'handPinkyA_L_IkCtrl', Nmsp + 'handPinkyB_L_IkCtrl', Nmsp + 'handRingA_L_IkCtrl',
                 ...
                 Nmsp + 'handAuxA_L_ikCtrl', Nmsp + 'handAuxB_L_ikCtrl', Nmsp + 'arm_L_BPoleVectorCtrl']
    ...
    rightIkArm = [Nmsp + 'footF_R_Ctrl', Nmsp + 'handPinkyA_R_IkCtrl', Nmsp + 'handPinkyB_R_IkCtrl', Nmsp + 'handRingA_R_IkCtrl',
                  ...
                  Nmsp + 'handAuxA_R_ikCtrl', Nmsp + 'handAuxB_R_ikCtrl', Nmsp + 'arm_R_BPoleVectorCtrl']
    ...
    leftIkLeg = [Nmsp + 'foot_L_Ctrl',  Nmsp + ' leg_L_BCtrl',  Nmsp + ' toe_L_BCtrl',  Nmsp + ' legThumbA_L_IkCtrl',
                 ...
                 Nmsp + 'legAuxA_L_ikCtrl', Nmsp + 'legAuxB_L_ikCtrl', Nmsp + 'leg_L_BPoleVectorCtrl']
    ...
    rightIkLeg = [Nmsp + 'foot_R_Ctrl',  Nmsp + ' leg_R_BCtrl',  Nmsp + ' toe_R_BCtrl',  Nmsp + ' legThumbA_R_IkCtrl',
                  ...
                  Nmsp + 'legAuxA_R_ikCtrl', Nmsp + 'legAuxB_R_ikCtrl', Nmsp + 'leg_R_BPoleVectorCtrl']
...
@undo_redo
# offsetを適用したbake
def moveCtrlOffsetBake(ui, *args):
    Nmsp = ui.nmspList.currentItem().text()
    Nmsp = Nmsp + ':'
    ...
    nums = ui.FrameOffsetValue.text()
    nums = float(nums)

    offsetStartA = sF + nums + 1
    offsetStartB = sF + nums + nums + 2
    offsetEndA = eF - nums - 1
    offsetEndB = eF - nums - nums - 1
...
@undo_redo
def moveCtrlAnimationBake(ui, *args):
    viewChange()
    ...
    pos = []
    if ui.HeadButton.isChecked():
        pos = 1
    elif ui.Spine_02Button.isChecked():
        pos = 2
    else:
        pos = 3
    ...
    if pos == 1:
        if setRot == 1:
            # time offsetを行わない場合
            if nums == 0:
                moveCtrlBake(ui, "head", "rot")
            # time offsetを行う場合
            else:
                moveCtrlOffsetBake(ui, "head", "rot")
        else:
            if nums == 0:
                moveCtrlBake(ui, "head", "trans")
            else:
                moveCtrlOffsetBake(ui, "head", "trans")
    # spineだったら
    elif pos == 2:
        if setRot == 1:
            # time offsetを行わない場合
            if nums == 0:
                moveCtrlBake(ui, "spine", "rot")
            # time offsetを行う場合
            else:
                moveCtrlOffsetBake(ui, "spine", "rot")
        else:
            if nums == 0:
                moveCtrlBake(ui, "spine", "trans")
            else:
                moveCtrlOffsetBake(ui, "spine", "trans")

    else:
        if setRot == 1:
            # time offsetを行わない場合
            if nums == 0:
                moveCtrlBake(ui, "spineAvr", "rot")
            # time offsetを行う場合
            else:
                moveCtrlOffsetBake(ui, "spineAvr", "rot")
        else:
            if nums == 0:
                moveCtrlBake(ui, "spineAvr", "trans")
            else:
                moveCtrlOffsetBake(ui, "spineAvr", "trans")
    viewChangeEnd()


@undo_redo
def setMoveCtrlAnimation(ui, *args):
    Nmsp = ui.nmspList.currentItem().text()
    Nmsp = Nmsp + ':'
