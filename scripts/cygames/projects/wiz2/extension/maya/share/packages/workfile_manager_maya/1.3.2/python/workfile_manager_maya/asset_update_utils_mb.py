...
class AssetUpdateUtils():
    ...
    def _check_update_proc_work_share(self, asset, masterfile, ch, ref_filename, updates, refs_new):
        ...
        if ch.area() == 'share':
            ...
            if len(buf) == 0:
                # Don't check below because there may be only unofficial versions after official feature introduced.
                #if not self.silent:
    ...
    def _proc_update(self, filename, updates, rfn_updates):
        ...
        if filename.endswith('.mb'):
            for org_rfn in cmds.ls(type='reference'):
                try:
                    old_ref = cmds.referenceQuery(org_rfn, filename=True)
                except:
                    continue
    ...
    def _proc_db_work(self, filename, asset, _asset, lines, refs_new, depth):
        from workfile_manager.ui import ui_table_work
        ...
        if filename.endswith('.mb'):
            ...
            if len(buf) > 0:
                ...
                if asset.version is None:
                    if asset.private_version is None:
                        ...
                        verstr = '%s%03d' % (ui_table_work.PRIVATE_VERSION_PREFIX, asset.private_version)
                else:
            ...
            if asset.is_updatable():
                ...
                res = task.save_workasset(_asset, refs_new, comment=comment, save_local=True, export_thumbnail=False)
                if res:
                    outfile, out_asset = res
                    print('saving workasset DONE!!!!!!!!!!!')
                    print('outfile, out_asset: ', outfile, out_asset)
                else:
            ...
            verstr = ('%s%03d' % (ui_table_work.PRIVATE_VERSION_PREFIX, asset.private_version)) if asset.private_version is not None else 'v%03d' % asset.version
            ...
            if filename.endswith('.mb'):
                ...
                if asset.is_updatable() or asset.is_reference_updatable():
                    ...
                    if asset.area() == 'share':
                        ...
                        outfile, out_asset = self._proc_db_work(filename, asset, _asset, lines, refs_new, depth)

                    elif asset.area() == 'engine':
                        outfile, out_asset = self._proc_db_engine(filename, asset, _asset, refs_new, depth)
