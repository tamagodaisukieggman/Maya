...
def is_version_already_used(asset, masterfile, latest_file, latest_ver):
    ...
    already_used = []
    if asset.area() == 'work':
        already_used = db.get_workasset_refs(filename=masterfile, ref_name=latest_file, ref_revision=latest_ver, include_omit=False)
...
class AssetUpdateUtils():
    ...
    def check_update(self, asset, filename, depth=0, check_self=False, parent_asset=None, silent=False):
        ...
        print('\t' * depth + 'check_update: ', filename)
        res = None
        if asset.area() == 'share' or asset.area() == 'work' or asset.area() == 'engine':
            res = self._check_update(asset, filename, depth=depth)
        else:
    ...
    def _check_update_proc_work_share(self, asset, masterfile, ch, ref_filename, updates, refs_new):
        ...
        if ch.area() == 'share':
            ...
            buf = db.get_sharedasset_versions(ch, ignore_omit=True)
            if len(buf) == 0:
                if not self.silent:
                    uiutils.PromptDialog('Error', u'参照しているファイルはデータベースから削除されています。\n\n'+ref_filename, btns=['OK']).exec_()
                return
        ...
        if latest_ver is not None and latest_file is not None:
            if latest_ver > cur_ver and os.path.exists(latest_file):
                ...
                if not already_used:
                    ...
                    if ref_filename in [x[0] for x in updates]:
                        ...
                        if self.update_all_later is None:
                            res = uiutils.PromptDialog('Confirmation',
                    ...
                    if res == 1:
                        ...
                        if ch.area() == 'work':
                            ...
                            task.open_workasset(ch, ref_filename, latest_ver)
    ...
    def _check_unused_new_revision(self, ref_filename, latest_ver, cur_ver, filestr):
        ...
        if self.update_all_later is None:
            ...
            d = uiutils.PromptDialog('Confirmation',
                ...
                if wrefs is not None and len(wrefs) == 0 and asset.is_reference_updatable():
                    ...
                    if res == 1:
                        ...
                        if task._diff_exists(diff):
                            if not self.silent:
                                uiutils.PromptDialog('Confirmation', u'未サブミットのローカルファイルが存在します。\n\nアップデート出来ません。', btns=['Continue']).exec_()
                        else:
    ...
    def _replace_path(self, line, old_path, new_path):
        ...
        if new_line != line:
            return new_line
        if assetutils.is_share_file(old_path):
    ...
    def _proc_update(self, filename, updates, rfn_updates):
        ...
        for i, update in enumerate(updates):
            if assetutils.is_share_file(update[1]): # False if candidates are cached share files.
    ...
    def _proc_db_share(self, filename, asset, _asset, lines, refs_org, refs_new, depth):
        db = assetdbutils.DB.get_instance()

        master = db.get_sharedasset_from_file(filename)[0]
        _asset.truncate('version')
        tags = [(x['tag_type'], x['name']) for x in db.get_assigned_tags(_asset)]
        _asset.version = db.next_available_shareasset_version(_asset, tags=tags)
        outversion = _asset.version
        ...
        db.publish(outfile, _asset, '%s [Updated from version:%d]' % (master['comment'], master['version']), username=p4u.p4.user, workfile=None, path_template=_asset.path_template)
        ...
        db.publish_refs(_asset, outfile, refs_new, p4u.p4.user, source_textures=source_textures)

        return outfile, outversion
        

    def _proc_db_work(self, filename, asset, _asset, lines, refs_new, depth):
        print('>>> _proc_db_work: asset: ', asset.get_dict())
        print('>>> _proc_db_work: _asset: ', _asset.get_dict())
        task = wcmds.Task(upload=True)
        #_asset.truncate('version')
        ...
        res = task.save_workasset(_asset, refs_new, comment='%s[Updated from version:%d]' % (original_comment, asset.version), 
    ...
    def _proc_db_engine(self, filename, asset, _asset, refs_new, depth):
        db = assetdbutils.DB.get_instance()
        _asset.truncate('version')
        ...
        _asset.version = None
        args['asset'] = _asset
        args['comment'] = '%s[Updated from version:%d]' % (original_comment, asset.version)
        args['user'] = p4u.p4.user
        outversion = db.commit_to_engine(revs, args)
    ...
    def _check_update(self, asset, filename, depth):
        from workfile_manager import ReturnState
        db = assetdbutils.DB.get_instance()
                ...
                if len(vers) > 0 and vers[0]['version'] > buf[0]['version']:
                    ...
                    if not already_used:
                        ...
                        if res == 1:
                            ...
                            _asset2.version = outversion
                        else:
                            dont_self_updated_selected = True
                        

            refs_org = db.get_share_asset_refs(_asset2, filename)


        elif asset.area() == 'work':
            refs_org = dbutils.DB.getrefs(filename, asset.version, private_version=asset.private_version if hasattr(asset, 'private_version') else None)
        elif asset.area() == 'engine':
        ...
        print ('\t' * depth + 'refs: ', refs)
        if len(refs) == 0:
            if outversion is None:
                return
            else:
                return {'outfile':outfile, 'outversion':outversion, 'rfn_updates':rfn_updates}
        ...
        if not dont_self_updated_selected:
            ...
            for ref in refs:
                ...
                if assetutils.is_share_file(ref_filename):
                    '''
        ...
        if len(updates) > 0:
            ...
            if asset.is_updatable() or asset.is_reference_updatable():
                _asset = copy.deepcopy(asset)
                if asset.area() == 'share':
                    outfile, outversion = self._proc_db_share(filename, asset, _asset, lines, refs_org, refs_new, depth)
                    
                elif asset.area() == 'work':
                    outfile, outversion = self._proc_db_work(filename, asset, _asset, lines, refs_new, depth)

                elif asset.area() == 'engine':
                    outfile, outversion = self._proc_db_engine(filename, asset, _asset, refs_new, depth)
