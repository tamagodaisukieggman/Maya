...
def replace_to_cache_online(rfn=None):
    ...
    if rfn is None:
        ...
        if not cmds.referenceQuery(rfn, isLoaded=True):
            ...
            if filename.startswith(croot):
                orgfile = cmds.get_share_path_from_cache(filename)
                wcmds.cache_sharefile(orgfile)
            else:
                share_root = assetdbutils.normalize_path(os.environ['WM_SHARE_ROOT'])
                if filename.startswith(share_root):
                    _, cached = wcmds.cache_sharefile(filename)
                    filename = cached

            cmds.file(filename, lr=rfn)
...
from workfile_manager.ui import ui_save_window
class AssetSpecificProperties(ui_save_window.AssetSpecificProperties):
...
class WorkAssetMaya(assetvar.WorkAsset):
    def asset_specific_ui_properties(self, table):
        return AssetSpecificProperties(table)
...
class ShareAssetMaya(assetvar.ShareAsset):
    ...
    def import_(self, filename, namespace_option=3, namespace=None, r=False, import_mode='Update', args=None):
        ...
        if r or import_mode == 'Append':
            cmds.file(filename, ignoreVersion=True, r=True, f=True, ns=import_namespace, pr=True, rfn=namespace+'RN', **args)
        else:
...
class TextureCopyWidget(QtWidgets.QWidget):
    def __init__(self, projdir, outside):
        ...
        for i in range(len(outside)):
            ...
            if select_outside and not os.path.dirname(dirname).startswith(workarea):
                for r in range(3):
    ...
    def execute(self):
        ...
        if not os.path.exists(w.projdir.text()):
            uiutils.PromptDialog('Error', 'Project directory not exists', btns=['OK']).exec_()
            return False
...
def check_texture_path():
    ...
    if len(notexists)>0:
        ...
        d = uiutils.PromptDialog('Warning', u'下記テクスチャーは存在しません。', wd=w, btns=['OK'])
        d.exec_()

    if len(outside)>0:
        help_url = 'https://wisdom.cygames.jp/pages/viewpage.action?pageId=397715457'
        rw = TextureCopyWidget(projdir, outside)
        d = uiutils.PromptDialog('Confirmation: Copy Textures', u'下記テクスチャーは保存アセットのMayaプロジェクト領域外です。\n\n' + \
...
def extract_embeded_scene_desc(filename):
    ...
    try:
        ...
        import traceback
        print(traceback.format_exc())
        uiutils.ErrorDialog(message=u'シーンファイルにパイプラインメタ情報が存在しません。\n\n' + filename+ '\n').exec_()
        return None
