...
def create_thumbnail(thumb):
    ...
    convcmd = 'W:/production/tools/cygames/thirdparty/win/standalone/FFmpeg/v4.0.2/bin/ffmpeg.exe -y -i %s.0000.png ' % (thumb_path) + \
...
class Task:
    def prepare_input(self):
        if self.inputfile.endswith('.ma'):
            ...
            assetutils_maya.replace_to_cache(self.inputfile)
    ...
    def write_tempfile(self, tmpname):
        if self.keep_intermediate:
            if self.inputfile is None:
                ...
                plugin_utils.write_tempfile(tmpname, 'xxx.ma')    
                return
            plugin_utils.write_tempfile(tmpname, self.inputfile)
    ...
    def execute(self, presetname=None, inputfile=None, argfile=None):
        ...
        self.user = args['user']
        print('post_fbx_export(maya): wait...', flush=True)
        ...
        if inputfile is None:
            if 'inputfile' in args:
                print('inputfile: ', args['inputfile'].replace('/', '\\'), flush=True)
        ...
        if 'sync_files' in args:
            for data in args['sync_files']:
                if data['area'] == 'work':
                    ...
                    print('workasset: ', buf, flush=True)
                    try:
        ...
        if inputfile is None:
            ...
            cmds.file(inputfile, ignoreVersion=True, f=True, o=True)
        ...
        try:
            ...
            if 'frame_rate' in args:
                print('setting frame rate: ', args['frame_rate'], flush=True)
                dccutils.set_framerate(args['frame_rate'])
        except:
            args['frame_rate'] = dccutils.get_framerate()
            print ('frame_rate not set. use as is.', flush=True)
            print('frame_rate:', args['frame_rate'], flush=True)
            
        try:
            assert args['frame_range'] is not None
            if 'frame_range' in args:
                print('frame_start: ', args['frame_range'][0], flush=True)
                print('frame_end: ', args['frame_range'][1], flush=True)
            ...
            print ('frame_range not set. use as is.', flush=True)
            print('frame_range: ', args['frame_range'], flush=True)
        ...
        if 'selection' in args and type(args['selection'])==list:
            ...
            print('selection key NOT exists.', flush=True)
            selection = []
        ...
        print('selected_ns: ', selected_ns, flush=True)

        for r in refs:
            try:
                ref_ns = cmds.referenceQuery(r, ns=True).strip(':')
            except:
                print('ERROR: Cannot find the associated namespace.', flush=True)
                continue
            print('ref_ns: ', ref_ns, flush=True)
            if ref_ns not in selected_ns:
                print('not selected reference. skip.', flush=True)
                continue
            ...
            if not assetutils.is_share_file(ref_filename) :
        ...
        print('deletable_ns: ', deletable_ns)
        ...
        print('postproc_outputs: ', postproc_outputs)
        ...
        if args['publish_app'] == Application.Maya:
            ...
            if os.path.exists(thumb):
                ...
                def is_source_on_share(thumb):
                    if 'WM_TMP_DIR' in os.environ:
                        if thumb.startswith(assetdbutils.normalize_path(os.environ['WM_TMP_DIR'])):
                            return False
                    if assetutils.is_share_file(thumb):
                        return True
                    return False

                WM_VER = os.environ['WORKFILE_MANAGER_VERSION']
                if assetdict['task'] == 'animation' and not is_source_on_share(thumb) and ('dont_overlay_icon' not in args or not args['dont_overlay_icon']):
                    play_icon = 'W:/production/tools/projects/world/inhouse/win/lib/python/share/packages/workfile_manager/%s/workfile_manager/ui/icons/playback.png' % WM_VER
                ...
                if not is_source_on_share(thumb) and not thumb.startswith(assetutils.get_resource_root()):
                    os.remove(thumb)
            ...
            if 'submit_server' in args and args['submit_server']:
                db.publish_refs(asset, outfile, refs, args['user'] if 'user' in args else p4u.p4.user, source_textures=source_textures)
            # Delete temporal files.
            if thumb and os.path.exists(thumb) and thumb != thumb_share and ('dont_delete_tmp_thumbnail' not in args or not args['dont_delete_tmp_thumbnail']) \
                    and not thumb.startswith(assetutils.get_resource_root()):
                os.remove(thumb)
            # update database
            #
            assetdict['success'] = 1
        ...
        if args['commit_to_engine']:
            ...
            db.replace_tags(asset=engine_asset, tags=tags, username=args['user'] if 'user' in args else p4u.p4.user)
            
            from workfile_manager.export.on_commit_proc import on_commit_proc
        ...
        try:
            if inputfile is not None and not assetutils.is_share_file(inputfile):
...
def postfunc(presetname=None, inputfile=None, argfile=None):
    ...
    if len(task.tracebacks) > 0:
        ...
        for tb in task.tracebacks:
            ...
            print(u'No errors exist except HTTPError.')
            return
        ...
        try:
            for user in ['takeuchi_kengo', 'tanaka_ko']:
                ...
                notif.send_to_slack(message, channel, mentions=['takeuchi_kengo', 'tanaka_ko'], is_channel=True, channel_mention=False, header=(p4u.p4.user, u'パブリッシュ時のエラー通知'))
