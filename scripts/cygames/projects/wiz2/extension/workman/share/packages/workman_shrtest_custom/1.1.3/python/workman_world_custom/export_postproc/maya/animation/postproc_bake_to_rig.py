...
def generate_ref_skeleton(root_jnt_name, filename=None):
    #
    # Notice:
    #   Suppose that a skeleton scene file is referenced without any namespace in the rig file.
    #
    ...
    if filename is not None:
        ...
        if filename == '' or filename is None:
            raise Exception('input filename not specified: ', filename)
    ...
    if not cmds.objExists(root_jnt_name):
        for n in cmds.ls():
            print('node: ', n)
        raise Exception('Root joint not exists: ' + root_jnt_name)
    ...
    # force to change the filename in MayaAscii format.
    tmpfile = tmpfile[:tmpfile.rindex('.')] + '.ma'
...
def find_src_rigfile(args):
    ...
    buf = db.cs.fetchall()
    if len(buf) == 0:
        raise Exception('Cannot find any work file as a source: ' + anim_file)
    work = buf[0]
    ...
    if len(buf) == 0:
        raise Exception('Cannot find any source rig: ' + anim_file)
    print('rig cand: ', buf)
...
class Plugin(PostProcBase):
    ...
    def execute(self, args):
        ...
        print('rigfile: ', rigfile, flush=True)
        ...
        try:
            ...
            if 'default_rig_skeleton' in args:
                ...
                raise Exception('default rig skeleton not set.')

        ref_skeleton_dst = generate_ref_skeleton('root_jnt')
        ref_skeleton_src = generate_ref_skeleton('root_jnt', filename=src_rigfile)
        print('ref_skeleton_dst:', ref_skeleton_dst, flush=True)
        print('ref_skeleton_src:', ref_skeleton_src, flush=True)
        ...
        if m:
            ...
            raise Exception('Not supported asset name.')
