...
def register_commit_job(anim_list_filename, deps, deadline_batchname, comment, output_overrides):
    ...
    package_version = '""'
    info = {'name':'Register workfiles', 'username':p4u.p4.user, 'task':share_asset.task, 'comment':'Submit generated workfiles.'}
...
def execute_commit(presetname=None, argfile=None):
    ...
    for tmp_output in anim_list.keys():
        ...
        if workasset is None:
            ...
            asset.path_template = asset.evaluate_output(out_config, proc_non_tag=False, proc_tag=True, tags=tags)
        else:
            for k in asset.get_dict():
        ...
        asset._format = outfile[outfile.rindex('.')+1:]
        dict_ = db.next_available_workasset_version(asset)
        asset.version = dict_['version']
