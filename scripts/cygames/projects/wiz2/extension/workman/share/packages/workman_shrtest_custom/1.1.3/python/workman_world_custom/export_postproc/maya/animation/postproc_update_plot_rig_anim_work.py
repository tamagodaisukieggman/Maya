...
class Plugin(PostProcBase):
    ...
    def execute(self, args):
        ...
        if not buf:
            print('filename not registered in db:', anim_file, flush=True)
            return
        
        src = buf[0]['source']
        if not src.endswith('.ma') and not src.endswith('.mb'):
            return

        src_version = buf[0]['source_version']

        if not src or not src_version:
            print('Source workfile not registered.', flush=True)
            return

        vers = db.get_workasset_versions_2([(src, src_version)])
        if len(vers) == 0:
            print('Invalid records or none.', flush=True)
            return
        ...
        print('workfile:', src)
        print('work revision:', work_revision, flush=True)
        ...
        print('framerange:', framerange, flush=True)
        ...
        fbxfile = re.sub('[.]m[ab]$', '.fbx', assetutils.get_publish_tempfilename_local(src))
        ...
        # Open rig
        print('Opening rig file: ', rig_file, flush=True)
        ...
        # Replace rigs
        print('Replacing rigs in: ', src, flush=True)
        workfile, rig_namespaces = self.replace_rigs(src, args['tmp_output'])
        print('Rigs replaced', flush=True)
        ...
        if len(rig_namespaces) == 0:
            ...
            rig_namespaces.append(model_asset.get_namespace())
        print('import animation namespace(s):', rig_namespaces)
        anim_asset = assetutils_maya.AnimationAssetMaya()

        cmds.FBXImportShowUI(v=False)
        for rig_ns in rig_namespaces:
            print('Importing animation to namespace:%s' % rig_ns)
            anim_asset.import_(fbxfile, namespace_option=2, namespace=rig_ns)
    ...
    def replace_rigs(self, org_workfile, outfile):
        print('replace_rigs: org_workfile: ', org_workfile, flush=True)
        print('replace_rigs: outfile: ', outfile, flush=True)
        
        rig_namespaces = []
        cmds.file(org_workfile, o=True, f=True)
        ...
        for rfn in cmds.ls(type='reference'):
            ...
            print('match: ', match)
            if match:
                print('Replacing %s to %s' % (ref, self.rig_file))
                cmds.file(self.rig_file, loadReference=rfn)
                ns = cmds.referenceQuery(rfn, ns=True)
                rig_namespaces.append(ns)

        cmds.file(rn=outfile)
        cmds.file(save=True, f=True)

        #if not org_workfile.endswith('.ma'):
        #    tmpfile = outfile[:outfile.rindex('.')] + '.ma'
        #    cmd = 'file -rn "%s";file -f -save -type mayaAscii;' % tmpfile
        #    import subprocess
        #    subprocess.call('mayabatch -file %s -command "%s"' % (org_workfile, cmd.replace('"', '\\"')))
        #    org_workfile = tmpfile
        #
        #    shutil.copyfile(tmpfile, 'C:/Users/S05231/Documents/tmp/test1.ma')

        '''
        ex = re.compile('^(.*")([a-zA-Z]:.*ma)(".*)$')
        nsex = re.compile('^file .* -ns "([^"]+)"')

        with open(org_workfile, 'r') as fhd:
            lines = fhd.readlines()
        ...
        shutil.copy(outfile, 'C:/Users/S05231/Documents/tmp')
        '''

        return outfile, rig_namespaces
