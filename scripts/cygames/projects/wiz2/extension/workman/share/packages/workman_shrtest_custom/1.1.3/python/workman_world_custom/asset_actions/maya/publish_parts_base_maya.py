...
class MAReader2():
    def read(self, filename):
        ...
        self.desc = assetutils_maya.extract_embeded_scene_desc(filename)


    def get_sets(self):
        sets = self.desc['sets']
        res = []
        for part_name in sets.keys():
            res.append((sets[part_name]['node_name'], part_name))
        return res

    def get_set_attribute(self, setname, pname, ptype=None):
        try:
            #print('object_sets: ', self.desc['object_sets'])
            params = [x for x in self.desc['object_sets'] if x['name']==setname][0]['params']
            #print('params: ', params)
            v = [x[1] for x in params if x[0]==pname][0]
            return v
        except:
            return None
    ...
    def get_operators(self, partnames):
        ...
        ops = []
        sets = self.desc['sets']
        for partname in sets.keys():
            if partnames is not None and partname not in partnames:
                continue
            ops_ = sets[partname]['operator']
            for op_ in ops_:
                name = op_['name']
                type_ = op_['type']
                op = operator.get_operator_from_typename(type_)
                ops.append((name, op))
        return ops


    def get_set_members(self, setname):
        object_sets = self.desc['object_sets']
        for object_set in object_sets:
            name = object_set['name']
            if setname != name:
                continue
            return object_set['members']
        return []

    def get_linked_objects(self, partname):
        sets = self.desc['sets']
        try:
            return sets[partname]['linked_objs']
        except:
            return []
    ...
    def calc_total_publish_cnts(self, parts, operators):
        ...
        for partname in sets.keys():
            ...
            for op_dict in sets[partname]['operator']:
