...
def bRivet():
    ...
    if input and len(input) >=2:  
        ...
        cmds.warning('bRivet requires at least 2 edges to be selected')
...
def bRivet_Build(input, ob, edgeMax, shape, count):
    ...
    io = ['input', 'output']
    uv = 'UV'        
    doublet = list(zip(list(range(count-1)),list(range(1, count))))
    locs = []
    node = 1
    ...
    for pair in doublet:
        ...
        gp = cmds.group(em = True, name = 'rivet%s_GP' % node )
        loc = cmds.spaceLocator(name = 'rivet%s' % node)   
