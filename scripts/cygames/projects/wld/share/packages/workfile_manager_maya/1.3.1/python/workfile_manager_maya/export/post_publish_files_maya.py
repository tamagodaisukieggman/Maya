...
def postfunc(presetname=None, inputfile=None, argfile=None):
    ...
    print('pid:', pid)
    print('maxproc:', maxproc, flush=True)
    for f in args['target_files'][pid::maxproc]:
        ...
        if workasset_dict is None:
            workasset_dict = {'assetgroup':'character', 'subcategory':'ply',
                                'assetname':'ply01', 'variant':'default', 'task':'animation',
                                'subgroup':'all', 'leafname':'default', 'localid':0, 'version':0, 'path_template':None, 'private_version':None,
                                'local_master_path':f}
        ...
        for k in work_asset.get_dict():
            setattr(work_asset, k, workasset_dict[k])
        ...
        for k in share_asset.get_dict():
            setattr(share_asset, k, workasset_dict[k])
        ...
        for part in args['publish_parts_parts']:
            ...
            print('setname: ', setname, flush=True)
            if setname is None:
                continue
            ...
            print('tmpoutfile: ', tmpoutfile.replace('/', '\\'), flush=True)
    ...
    print('_argfile: ', _argfile.replace('/', '\\'), flush=True)
