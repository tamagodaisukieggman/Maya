...
def connect_hik(namespace='male00_all1000_mdl_def:', gender='male', import_joints_filePath=None, import_xml_filePath=None, anim_joints_filePath=None, anim_xml_filePath=None):

    not_human_assets = ['pod']

    if cmds.namespace(ex='import'):
        cmds.namespace(rm='import', dnc=1, f=1)


    cmds.file("{0}".format(import_joints_filePath), pr=1, ignoreVersion=1, i=1, importTimeRange="combine", mergeNamespacesOnClash=False, options="v=0;", namespace='import')
    cmds.file("{0}".format(anim_joints_filePath), pr=1, ignoreVersion=1, i=1, importTimeRange="combine", mergeNamespacesOnClash=False, options="v=0;")

    if not gender in not_human_assets and not import_joints_filePath == anim_joints_filePath:
    ...
    elif gender == 'pod' and import_joints_filePath == anim_joints_filePath:
        cmds.setAttr('{}handL_ikfk_ctl.ikfk'.format(namespace), 0)
        cmds.setAttr('{}handR_ikfk_ctl.ikfk'.format(namespace), 0)
    ...
    def matchConstraint(currentNamespace='', jointNamepace='', gender='male'):
        def constraint_convert(src, dst, pos, rot, scl, mo):
            ...
            if pos:
                ...
                cnsts.append(cnst)
            if rot:
                if not gender in not_human_assets:
                    ...
                    if ('spine_02_ctl' in dst
                        or 'neck_ctl' in dst
                        or 'shoulderL_ctl' in dst):
                        cnst = cmds.orientConstraint(src, dst, w=1, mo=1, sk=['y', 'z'])
                        cnsts.append(cnst)

                    if ('handL_rot_ctl' in dst
                        or 'handR_rot_ctl' in dst
                        or 'fk_handL_ctl' in dst):
                        cnst = cmds.orientConstraint(src, dst, w=1, mo=1, sk=['x', 'z'])
                        cnsts.append(cnst)

                    if 'spine_01_ctl' in dst:
                        cnst = cmds.orientConstraint(src, dst, w=1, mo=1)
                        cnsts.append(cnst)

                    if ('fk_armL_ctl' in dst):
                        cnst = cmds.orientConstraint(src, dst, w=1, mo=1, sk=['y'])
                        cnsts.append(cnst)
                    ...
                    # aim
                    if ('shoulderR_ctl' in dst):
                        arm_aim_loc = cmds.spaceLocator()
                        cmds.parent(arm_aim_loc[0], src)
                        cmds.xform(arm_aim_loc[0], t=[0,-20,0], ro=[0,0,0], os=1, a=1)
                        cnst = cmds.aimConstraint(arm_aim_loc[0], dst, weight=1, upVector=(0, -1, 0), sk=['y', 'z'], worldUpType="vector", offset=(0, 0, 0), aimVector=(0, -1, 0), worldUpVector=(0, 1, 0))
                        cnsts.append(cnst)


                    if ('fk_armR_ctl' in dst):
                        arm_aim_loc = cmds.spaceLocator()
                        cmds.parent(arm_aim_loc[0], src)
                        cmds.xform(arm_aim_loc[0], t=[0,-20,0], ro=[0,0,0], os=1, a=1)
                        cnst = cmds.aimConstraint(arm_aim_loc[0], dst, weight=1, upVector=(0, -1, 0), skip='y', worldUpType="vector", offset=(0, 0, 0), aimVector=(0, -1, 0), worldUpVector=(0, 1, 0))
                        cnsts.append(cnst)

                    if ('fk_forearmR_ctl' in dst):
                        arm_aim_loc = cmds.spaceLocator()
                        arm_wuo_loc = cmds.spaceLocator()
                        cmds.parent(arm_aim_loc[0], src)
                        cmds.parent(arm_wuo_loc[0], src)
                        cmds.xform(arm_aim_loc[0], t=[20,0,0], ro=[0,0,0], os=1, a=1)
                        cmds.xform(arm_wuo_loc[0], t=[0,-20,0], ro=[0,0,0], os=1, a=1)
                        # cnst = cmds.aimConstraint(arm_aim_loc[0], dst, weight=1, upVector=(0, -1, 0), worldUpType="vector", offset=(0, 0, 0), aimVector=(-1, 0, 0), worldUpVector=(0, 1, 0), wuo=arm_wuo_loc[0])
                        cnst = cmds.aimConstraint(arm_aim_loc[0], dst, weight=1, upVector=(0, -1, 0), worldUpObject=arm_wuo_loc[0], worldUpType="objectrotation", offset=(0, 0, 0), aimVector=(-1, 0, 0), worldUpVector=(0, -1, 0))
                        cnsts.append(cnst)

                    if ('fk_handR_ctl' in dst):
                        arm_aim_loc = cmds.spaceLocator()
                        cmds.parent(arm_aim_loc[0], src)
                        cmds.xform(arm_aim_loc[0], t=[20,0,0], ro=[0,0,0], os=1, a=1)
                        cnst = cmds.aimConstraint(arm_aim_loc[0], dst, weight=1, upVector=(0, -1, 0), sk=['x', 'z'], worldUpType="vector", offset=(0, 0, 0), aimVector=(-1, 0, 0), worldUpVector=(0, 1, 0))
                        cnsts.append(cnst)
        ...
        def constraint_convert_ik_pv(dst, pos, rot, scl, mo, start, mid, end, move):
            ...
            if ('forearmL_ctl' in dst
                ...
                if 'L_' in dst:
                    wuo = 'import:uplegL_jnt'

                if 'R_' in dst:
                    wuo = 'import:uplegR_jnt'

                cmds.aimConstraint(mid, loc2, w=1, aim=(0,0,1), u=(0,1,0), wut='objectrotation', wu=(0,1,0), wuo=wuo)
                cmds.move(0, 0, move, loc3, r=1, os=1, wd=1)

            cmds.orientConstraint(wuo, loc1[0], w=1, mo=1)
            # cmds.parentConstraint(wuo, loc1[0], w=1, mo=1)
            ...
            hip_cnst_sets = 'hip_cnst_sets'
            if not cmds.objExists(hip_cnst_sets):
                cmds.sets(em=1, n=hip_cnst_sets)

            cmds.sets(loc1[0], add=hip_cnst_sets)
        ...
        for jnt in source_joints:
            if cmds.objExists(jointNamepace+jnt):
        ...
        for ctrl, jnt_value in match_ctrls.items():
            if cmds.objExists(ctrl):
                ...
                try:
                    ...
                    #print(e) deadline上だと文字コードの例外を発生させてしまうためコメントアウト
                    import traceback
    ...
    elif not gender in not_human_assets and import_joints_filePath == anim_joints_filePath:
        import_joints = cmds.ls('import:root_jnt', dag=1, type='joint')
        for ijt in import_joints:
            try:
                cmds.parentConstraint(ijt.replace('import:', ''), ijt, w=1, mo=1)
            except Exception as e:
                import traceback
                print(traceback.format_exc())
    ...
    # --------------
    # root constraint
    pac = cmds.parentConstraint('root_jnt', '{}root_pos_ctl'.format(namespace), w=1, mo=1)
    cmds.sets(pac, add='bake_cnst_sets')

    # cog constraint
    pac = cmds.parentConstraint('cog_jnt', 'import:cog_jnt', w=1, mo=1)
    cmds.sets(pac, add='bake_cnst_sets')

    # hip constraint
    if cmds.objExists('hip_cnst_sets'):
        cmds.select('hip_cnst_sets', r=1, ne=1)
        hip_source = cmds.pickWalk(d='down')
        for hip_s in hip_source:
            pass
            # try:
            #     oric = cmds.orientConstraint('import:hip_jnt', hip_s, w=1, mo=1)
            #     cmds.sets(oric, add='bake_cnst_sets')
            # except:
            #     pass

    # joints sets
    hik_joints_sets = 'hik_joints_sets'
    if not cmds.objExists(hik_joints_sets):
        cmds.sets(em=1, n=hik_joints_sets)

    [cmds.sets(ct, add=hik_joints_sets) for ct in cmds.ls(assemblies=1) if cmds.objectType(ct) == 'joint']

    return 'bake_cnst_sets', hik_joints_sets
...
"""
# Import Joints
import_joints_filePath = 'C:/Users/CF0990/Documents/maya/scripts/tkgTools/tkgRig/data/projects/www/scripts/convert/hik/data/world/male00/joints.ma'
filename, file_extension = os.path.splitext(import_joints_filePath)
if file_extension == '.ma' or file_extension == '.mb':
    import_xml_filePath = import_joints_filePath.replace(file_extension, '.xml')

# Animation Joints
anim_joints_filePath = 'C:/Users/CF0990/Documents/maya/scripts/tkgTools/tkgRig/data/projects/www/scripts/convert/hik/data/world/male00/old_joints/joints.ma'
filename, file_extension = os.path.splitext(anim_joints_filePath)
if file_extension == '.ma' or file_extension == '.mb':
    anim_xml_filePath = anim_joints_filePath.replace(file_extension, '.xml')

bake_cnst_sets, hik_joints_sets = connect_hik(namespace='male00_all1000_mdl_def:',
            gender='male',
            import_joints_filePath=import_joints_filePath,
            import_xml_filePath=import_xml_filePath,
            anim_joints_filePath=anim_joints_filePath,
            anim_xml_filePath=anim_xml_filePath)
...
# Import Joints
import_joints_filePath = '{}/hik/data/world/male00/joints.ma'.format('/'.join(file_path.split('/')[0:-2:]))
filename, file_extension = os.path.splitext(import_joints_filePath)
if file_extension == '.ma' or file_extension == '.mb':
    import_xml_filePath = import_joints_filePath.replace(file_extension, '.xml')

# Animation Joints
anim_joints_filePath = '{}/hik/data/world/male00/old_joints/joints.ma'.format('/'.join(file_path.split('/')[0:-2:]))
filename, file_extension = os.path.splitext(anim_joints_filePath)
if file_extension == '.ma' or file_extension == '.mb':
    anim_xml_filePath = anim_joints_filePath.replace(file_extension, '.xml')


bake_cnst_sets, hik_joints_sets = rchh.connect_hik(namespace='male00_all1000_mdl_def:',
                                                    gender='male',
                                                    import_joints_filePath=import_joints_filePath,
                                                    import_xml_filePath=import_xml_filePath,
                                                    anim_joints_filePath=anim_joints_filePath,
...
# Import Joints
import_joints_filePath = 'C:/Users/CF0990/Documents/maya/scripts/tkgTools/tkgRig/data/projects/www/scripts/convert/hik/data/world/pod00/joints.ma'
filename, file_extension = os.path.splitext(import_joints_filePath)
if file_extension == '.ma' or file_extension == '.mb':
    import_xml_filePath = import_joints_filePath.replace(file_extension, '.xml')

# Animation Joints
anim_joints_filePath = 'C:/Users/CF0990/Documents/maya/scripts/tkgTools/tkgRig/data/projects/www/scripts/convert/hik/data/world/pod00/joints.ma'
filename, file_extension = os.path.splitext(anim_joints_filePath)
if file_extension == '.ma' or file_extension == '.mb':
    anim_xml_filePath = anim_joints_filePath.replace(file_extension, '.xml')

bake_cnst_sets, hik_joints_sets = connect_hik(namespace='pod00_all1000_mdl_def:',
            gender='pod',
            import_joints_filePath=import_joints_filePath,
            import_xml_filePath=import_xml_filePath,
            anim_joints_filePath=anim_joints_filePath,
...
# Animation Joints
anim_joints_filePath = 'C:/Users/CF0990/Documents/maya/scripts/tkgTools/tkgRig/data/projects/www/scripts/convert/hik/data/world/male00/joints.ma'
filename, file_extension = os.path.splitext(anim_joints_filePath)
if file_extension == '.ma' or file_extension == '.mb':
    anim_xml_filePath = anim_joints_filePath.replace(file_extension, '.xml')
