...
class ViewPortCallBackRegister(BaseCallBackRegister):
    ...
    @classmethod
    def register(cls):
        cmds.condition(CUSTOM_VIEWPORT_CALLBACK, dependency="playingBack", script="python(\"import mtk; mtk.cutscene.editor.callback.ViewPortCallBackRegister.set_focus()\")")

        cls.camera_change_callback_id = cmds.scriptJob(event=["modelEditorChanged", "import mtk; mtk.cutscene.editor.callback.ViewPortCallBackRegister.save_viewport_camera()"])

        cmds.condition(CUSTOM_VIEWPORT_CAMERA_ATTRIBUTE_CALLBACK, dependency="cameraDisplayAttributesChange", script="python(\"import mtk;mtk.cutscene.editor.CutsceneEditorRegister.viewport_controller.view_grid_drawermanager.refresh_grid_all()\")")
