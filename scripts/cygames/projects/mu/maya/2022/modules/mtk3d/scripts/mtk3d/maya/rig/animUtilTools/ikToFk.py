...
class IkToFk(QDialog):
    def __init__(self, parent=None):
        ...
        self.UI = QUiLoader().load(os.path.join(CURRENT_PATH, "ikToFk.ui"), self)
    ...
    def iktofk(self, startFrame=None, endFrame=None):
        ...
        for i in range(len(self.switch)):
            self._setvalue_zero(self.switch[i], ["ikFkSwitch"], dc=True)

        # -------------------------------------------------------------------------------------------------------------#
        # make fk control dummy and constraint fk , add baked list
        for control in self.fk.viewkeys():
        ...
        if not val:
            for control in self.hoof_fk.viewkeys():
        ...
        for i in range(len(self.ik_space)):
            self._setvalue_zero(self.ik_space[i], ["space"], dc=True)
        ...
        for l in self.ik.viewvalues():
            for i in range(len(l)):
                self._setvalue_zero(l[i], ["tx", "ty", "tz", "rx", "ry", "rz"], dc=True)

        if not val:
            for l in self.hoof_ik.viewvalues():
                for i in range(len(l)):
                    print(l[i])
                    self._setvalue_zero(l[i], ["tx", "ty", "tz", "rx", "ry", "rz"], dc=True)
    ...
    def switchspace(self, startFrame=None, endFrame=None, replace=False):
        ...
        try:
            ...
            mc.bakeResults(baked.values(), t=(startFrame, endFrame), sm=True,
                           at=["t", "tx", "ty", "tz", "r", "rx", "ry", "rz"])
