...
#-------------------------------------------------
#メイン
def detach(mainFlg=0):
    if mainFlg == 1:
        print("")
        print("#-----")
        print("#[Python Command]")
        print("#import TkgDetachPolygon;reload(TkgDetachPolygon);TkgDetachPolygon.detach()")
        print("#-----")
        print("")
    ...
    for currentPolyName in allSelPolyNames:
        ...
        if not oldTransformName in meshPolyInfo:
            #メッシュを複製
            newTransformName = mc.duplicate(oldTransformName, name=(oldTransformName.split("|")[-1] + "_2"))[0]
