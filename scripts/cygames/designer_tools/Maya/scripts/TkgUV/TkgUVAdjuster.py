...
import TkgConvertSelection
from TkgComponent import getSelItemPosInfo

import TkgOpenWebPage;reload(TkgOpenWebPage)
    ...
    def __init__(self):
        ...
        with self.window:
            with pm.columnLayout(columnOffset=("left", 3)):
                pm.menuBarLayout()
                pm.menu(label="Help")
                pm.menuItem(label=u"ヘルプを開く", c=pm.Callback(TkgOpenWebPage.open, "https://wisdom.tkgpublic.jp/display/designersmanual/Maya%3A+UVAdjuster"))
...
class MappingPanel(object):
    ...
    def projectUV(self, mode):
        ...
        TkgConvertSelection.convert("polygon")
