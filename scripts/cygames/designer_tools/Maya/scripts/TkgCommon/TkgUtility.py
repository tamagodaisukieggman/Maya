...
class TkgUtility:
    ...
    def RemoveNamespace(name):
        ...
        if name.find("|") == -1:
            return TkgUtility.RemoveNamespaceBase(name)

        splitStrList = name.split("|")
        resultName = ""
        for p in range(0,len(splitStrList)):
        
            fixName = TkgUtility.RemoveNamespaceBase(splitStrList[p])
    ...
    def GetShortName(name):
        ...
        longName = TkgUtility.GetLongName(name)
    ...
    def SameValueList(valueList0, valueList1):
        ...
        for p in range(len(valueList0)):
            ...
            if TkgUtility.SameValue(valueList0[p], valueList1[p]) == False:
                return False
