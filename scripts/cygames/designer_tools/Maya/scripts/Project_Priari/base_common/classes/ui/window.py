    ...
    def set_job(self, event_or_conditon_name, function, *arg):
        ...
        if is_event:
            this_job = cmds.scriptJob(
                event=[event_or_conditon_name, _partial(self.__execute_job_function, this_index)], protected=True)
        else:
            this_job = cmds.scriptJob(
                conditionTrue=[event_or_conditon_name, _partial(self.__execute_job_function, this_index)], protected=True)
