...
from ..priari_common import classes as priari_class
from ..priari_common import utility as priari_utility
...
def check_mesh_poly_sum_count_from_head(param_item, target_sum_poly_min_count, arg):
    ...
    body_finder = priari_class.path_finder.path_finder.PathFinder('body', param_item.main.chara_info.data_main_id)
    ...
    for target_body_file in target_body_file_list:
        ...
        tmp_info_body = priari_common.classes.info.chara_info.CharaInfo()
        ...
        body_finder = priari_class.path_finder.path_finder.PathFinder('body', '{}_{}'.format(check_dict['body_part_info'].main_id, check_dict['body_part_info'].sub_id))
        tail_finder = priari_class.path_finder.path_finder.PathFinder('tail', '{}_{}'.format(check_dict['tail_part_info'].main_id, check_dict['tail_part_info'].sub_id))
...
def check_mesh_poly_sum_count_from_body(param_item, target_sum_poly_min_count, arg):
    ...
    head_finder = priari_class.path_finder.path_finder.PathFinder('head', '{}_{}'.format(check_dict['head_part_info'].main_id, check_dict['head_part_info'].sub_id))
    tail_finder = priari_class.path_finder.path_finder.PathFinder('tail', '{}_{}'.format(check_dict['tail_part_info'].main_id, check_dict['tail_part_info'].sub_id))
...
def check_neck_edge_normals(param_item, arg):
    ...
    neck_normal_info = priari_class.neck_normal.NeckNormalInfo()
    ...
    if texture_type == 'texture':
        ...
        for material in material_list:
            target_texture_list.extend(
                priari_utility.model_texture_finder.get_texture_list_from_material(material)
            )
...
def check_texture_size(param_item, arg):
    ...
    for texture in target_texture_list:
        ...
        target_size = priari_utility.model_texture_finder.get_target_texture_size(texture)
...
# ==================================================
def check_duplicated_locator_exist(param_item, arg):
    """
    重複するロケータが存在するかのチェック

    別階層に同名ロケーターが存在していないかのチェック
    本来なら命名則等から適切な階層かを判定するべきだが、ATCLロケーター数が不定なためパターン判定が必要になるのと
    今後priariの開発の予定はないため、簡易的に重複をチェックする
    """
    ...
    def is_locator(node): return cmds.listRelatives(node, s=True, typ='locator')
    locator_list = [n for n in cmds.listRelatives(param_item.main.chara_info.part_info.root_node, ad=True, f=True) if is_locator(n)]
    ...
    param_item.info_target_list.extend(locator_list)
    ...
    param_item.check_target_list = locator_list

    short_name_dict = {}
    for locator in locator_list:

        this_short_name = locator.split('|')[-1]
        if short_name_dict.get(this_short_name):
            short_name_dict[this_short_name].append(locator)
        else:
            short_name_dict[this_short_name] = [locator]
    
    for locator_list in short_name_dict.values():
        if len(locator_list) == 1:
            param_item.unerror_target_list.extend(locator_list)
        else:
            param_item.error_target_list.extend(locator_list)
...
def check_cloth_joint_sum_count_from_head(param_item, target_min_count, arg):
    ...
    for check_dict in check_dict_list:
        ...
        tmp_info = priari_common.classes.info.chara_info.CharaInfo()
...
def check_cloth_joint_sum_count_from_body(param_item, target_min_count, arg):
    ...
    head_finder = priari_class.path_finder.path_finder.PathFinder('head', '{}_{}'.format(head_info.main_id, head_info.sub_id))
