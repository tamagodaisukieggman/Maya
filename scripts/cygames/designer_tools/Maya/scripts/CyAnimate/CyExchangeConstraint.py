...
def UI():
    ...
    cmds.checkBox(uiPrefix + "CheckBoxFilter",label="元のコンストレイント名を保持する"
                          )
...
def exchangeConstraintRoot():
    ...
    for cnt in range (len(SelectList)):
        ...
        if constraintObjectType == "pointConstraint":
            ...
            print("ERROR:コンストレイントでないオブジェクトか、対応していないコンストレイントが選択されています。")
            return
...
def exchangeConstraint(constraintName,constraintType):
    ...
    if len(ConstraintWeightList) > 1:
        print("ERROR:一対一のコンストレイントのみ交換できます")
        return
    ...
    #アトリビュートがロックされていた際のエラー回避
    axisLock = getAttrlocked(masterName,settingNewConstraint,constraintType)
    if axisLock == False:
        print("ERROR:目的のアトリビュートがロックされています")
        return
    
    #pointConstarint、orientConstarintの親がparentConstraintに接続されていた際のエラー回避
    ...
    if masterConstraintInfo == False:
        print("ERROR:オブジェクトはすでに接続されています。")
        return
...
#-------------------------------------------------------------------------------------------
#   目的アトリビュートのロック情報（アトリビュートがロックされてる時のエラー回避
#-------------------------------------------------------------------------------------------
