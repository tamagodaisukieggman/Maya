...
#define FarmObjectToClipPos(a) mul(UNITY_MATRIX_MVP,a)
...
uniform float4 _High0Tex_ST;
uniform sampler2D _High1Tex;
uniform sampler2D _High2Tex;
...
v2f vert(appdata_t v)
{
    ...
    o.pos = FarmObjectToClipPos(inVertex);

#ifdef TOON_EYE
    float4 posWorld = mul(unity_ObjectToWorld, inVertex);
    ...
    normal = lerp(mul((float3x3)UNITY_MATRIX_M, normal), cylinderNormal, _CylinderBlend);
...
#ifdef USE_FOG
    o.fogParam = CalcFogParamOriginFromVertex(inVertex);
#endif
...
fixed4 frag(v2f i) : COLOR
{
    ...
    col.rgb *= lerp(half3(1, 1, 1), _MaskColorR1, r1);
    col.rgb *= lerp(half3(1, 1, 1), _MaskColorR2, r2);
    col.rgb *= lerp(half3(1, 1, 1), _MaskColorG1, g1);
    col.rgb *= lerp(half3(1, 1, 1), _MaskColorG2, g2);
    col.rgb *= lerp(half3(1, 1, 1), _MaskColorB1, b1);
    col.rgb *= lerp(half3(1, 1, 1), _MaskColorB2, b2);
#endif // USE_MASK_COLOR

    kira0 = tex2D(_High0Tex, i.uv0).r * i.kiraParam.x;
    kira0 = kira0 - _Limit;
    kira1 = -kira0;
    ...
    kira0 = tex2D(_High1Tex, i.uv1).r * i.kiraParam.y;
    kira0 = kira0 - _Limit;
    kira1 = -kira0;
    if (kira0 < 0.0)
    {
...
#if defined(MAYA_CGFX)
    // Maya用のコード
    kira0 = tex2D(_High2Tex, i.uv2).r * _HighParam12.w;
#else
    // Unity用のコード
    kira0 = tex2D(_High2Tex, i.uv2).r * _HighParam1[2].w;
#endif
    kira0 = kira0 - _Limit;
    kira1 = -kira0;
    if (kira0 < 0.0)
...
#if defined(MAYA_CGFX)
    // Maya用のコード
    half3 lightDirection = lerp(normalize(_WorldSpaceLightPos0.xyz), normalize(_OriginalDirectionalLightDir), step(1, (float)_UseOriginalDirectionalLight));
#else
    // Unity用のコード
    half3 lightDirection = lerp(normalize(_WorldSpaceLightPos0.xyz), normalize(_OriginalDirectionalLightDir), step(1, _UseOriginalDirectionalLight));
#endif
