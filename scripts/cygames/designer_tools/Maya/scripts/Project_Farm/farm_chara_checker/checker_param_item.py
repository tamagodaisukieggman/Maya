...
class CheckerParamItem(object):
    ...
    # ==================================================
    def __init__(self, root, param_value):
    ...
    def update_enable_flag(self):
        ...
        if self.main.chara_info.exists:
            ...
            data_id = self.main.chara_info.data_id

        if data_type:

            self.is_top_enable = True

            if self.data_type_list:

                self.is_top_enable = False

                if data_type.endswith('avatar'):

                    if 'avatar' in self.data_type_list:
                        self.is_top_enable = True

                elif data_type.endswith('unit'):

                    if 'unit' in self.data_type_list:
                        self.is_top_enable = True

                elif data_type.endswith('weapon'):

                    if 'weapon' in self.data_type_list:
                        self.is_top_enable = True

                elif data_type.endswith('prop'):

                    if 'prop' in self.data_type_list:
                        self.is_top_enable = True

                elif data_type.endswith('summon'):

                    if 'summon' in self.data_type_list:
                        self.is_top_enable = True

                elif data_type.endswith('enemy'):

                    if 'enemy' in self.data_type_list:
                        self.is_top_enable = True
