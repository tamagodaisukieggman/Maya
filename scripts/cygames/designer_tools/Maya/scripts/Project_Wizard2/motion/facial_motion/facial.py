...
def set_uv(face_mesh, face_part, faceIndex, u, v, checkbox_key):
    ...
    :param checkbox_key: QtWidgets.QCheckBox
    """
    set_key = False
    ...
    try:
        ...
        if set_key:
            ...
            print('キーフレームを設定しました: ' + str(texture2d) + ', ' + str(faceIndex) + ': ' + str(u) + ', ' + str(v))
    except Exception as ex:
            ...
            try:
                ...
                if face_part == rowPart:
                    btn_img_path = os.path.join(os.path.join(CURRENT_PATH, 'images'),
                                                csv_row[label_index_dict['Default Icon Image']])
...
def get_face_id_from_uv(face_part, u, v, texture_num, csv_rows):
    ...
    for row, csv_row in enumerate(csv_rows):
        if row == 0:
            ...
            tex_num = csv_row[label_col_dict['Texture Num']]
            try:
                ...
                cmds.error(ex)
                continue
            row_part = csv_row[label_col_dict['Face Part']]
            if row_part == face_part:
                try:
                    row_u = float(csv_row[label_col_dict['Translate Frame U']])
                    row_v = float(csv_row[label_col_dict['Translate Frame V']])
                except Exception:
                    cmds.error('facial_uv.csvからの表情番号取得失敗')
                    continue
                if row_u == u and row_v == v:
                    try:
                        face_id = int(csv_row[label_col_dict['Face ID']])
                        return face_id
                    except Exception as ex:
            ...
            if numKeyframes == 0:
                ...
                for i, u_val in enumerate(uValues):
                    ...
                    if not face_id:
                        ...
                        dict = {}
                        dict['Event'] = g_facial_event_id[face_part]
...
def get_facial_shapes_by_size(eyebrow_mesh, size, do_remove_namespace):
    ...
    if not cmds.objExists(eyebrow_mesh):
        return
    if not size or size not in ['S', 'M', 'L']:
        return
    try:
        target_shapes = cmds.blendShape(eyebrow_mesh, q=True, target=True)
    except Exception as ex:
