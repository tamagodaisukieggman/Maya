...
class Wiz2EnvTextureName(CheckTaskBase):
    ...
    def check_texture_name_pattern(self, texture, bg_type):
        ...
        if not self.env_dict.get('env_root_' + bg_type, False) or not self.env_dict.get('name_parts_' + bg_type, False):
            self.set_error_data(
                'env_unknown_bg_type', os.path.basename(texture), '登録外のロケーション: ' + str(bg_type), is_reset_debug_data=False
            )
            return
        ...
        for idx, part in enumerate(self.env_dict['name_parts_' + bg_type]):
            ...
            try:
                ...
                if not matchObj:
                    ...
                    self.set_error_data(
                        error_name, os.path.basename(texture), error_message, is_reset_debug_data=False
                    )
                else:
                    # 登録されたキーワードか
