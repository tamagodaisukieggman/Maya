...
from ..maya_checker.checker_factory import CheckerFactory
...
class CheckerMainWindow(MayaQWidgetDockableMixin, QtWidgets.QMainWindow):
    def __init__(self, parent=None, **kwargs):
        ...
        self.character_info = kwargs
        checker_helps = self.character_info["helps"]
        checker_tools = self.character_info["tools"]
        self.checker_settings = self.character_info["checker_settings"]

        # post_process_settingsはデフォルトの設定でも使用できるように
        self.post_process_settings = []
        if "post_process_settings" in self.character_info:
            self.post_process_settings = self.character_info["post_process_settings"]
    ...
    def initialize(self):
        ...
        # extradataの作成
        self._set_extra_data(self.character_info["tasks"], current_task_names, extra_datas)

        self.checker = CheckerFactory.create(self.root_nodes,self.character_info)

        self._initialized_task_table(self.character_info["tasks"])
...
def show(**kwargs) -> CheckerMainWindow:
    """Windowの起動

    Returns:
        CheckerMainWindow: 起動したmain windowのインスタンス
    """
    ...
    workspace_control_name = tool_title + "WorkspaceControl"
    if cmds.window(workspace_control_name, exists=True):
        cmds.deleteUI(workspace_control_name)

    ui = CheckerMainWindow(**kwargs)
    ui.setWindowTitle(tool_title)
    ui.setObjectName(tool_title)
