...
class Wiz2ProSceneLocation(CheckTaskBase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.checker_info.label_name = "シーンの場所"

    def exec_task_method(self):
        base_name = self.maya_scene_data.basename
        prop_type = ""
        if "_c_" in base_name:
            prop_type = "costume"

        elif "_g_" in base_name:
            prop_type = "general"
        ...
        elif prop_type == "general":
            if general_path not in self.maya_scene_data.name:
                is_valid_path = False
...
class Wiz2ProFileName(CheckTaskBase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def check_string_format(s: str) -> bool:
        ...
        """
        pattern = r'^pro_[gc]_.*\d{2}$'
        return bool(re.match(pattern, s))

class Wiz2ProMeshName(CheckTaskBase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def check_string_format(s: str) -> bool:
        ...
        Returns:
            bool: 指定された規則に従っている場合はTrue、それ以外はFalse
        """
        pattern = r'^pro_[gc]_.*\d{2}_\d{2}$'
        return bool(re.match(pattern, s))
    

class Wiz2ProJointName(CheckTaskBase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
...
class Wiz2ProRootGroupName(CheckTaskBase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def exec_task_method(self):
        ...
        error_groups = []
        for root in self.maya_scene_data.root_nodes:
            if root.short_name == "pro":
                continue

            if root.short_name.startswith("pro"):
                pattern = r'^pro_\d{2}$'
...
class Wiz2PrpHasDuplicateNodeNames(HasDuplicateNodeNames):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
