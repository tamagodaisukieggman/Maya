...
class Wiz2WepPolyCount(Wiz2PolyCount):
    ...
    def exec_task_method(self):
        ...
        if has_error == True:
            ...
            self.set_error_data(
                f"wiz2_weapon_poly_count",
                debug_datas["check_targets"],
                f"weapon の規定ポリゴン数({current_max_count}tris)内です。\n現在の合計ポリゴン数は{debug_datas['total_poly_count']}tris です。",
                is_reset_debug_data=True,
            ) 