...
class EdgeSetModifier:
    ...
    def apply_vertex_normal(self, vertex: str, vertex_info_file: str):
        ...
        for close,current in zip(closest_info["normal"], current_normal):
            if not compare_xyz(close,current):
                # polyinfoで取得したフェース番号の逆順がpolyNormalPerVertexの法線のquery
                current_vfs = self.extract_numbers(cmds.polyInfo(vertex,vf=True)[0])
                current_vfs.reverse()
                if len(current_vfs) != len(closest_info["normal"]):
                    cmds.warning(f"{vertex}に接続されているコンポーネント数が異なります。値を設定することができません。")
                    continue

                for i,current_vf in enumerate(current_vfs):
                    vf_name = vertex.replace("vtx","vtxFace")+f"[{current_vf}]"
                    # 対象頂点に最も近い頂点の法線を適用します
                    print("適用",vf_name, closest_info["normal"][i])
                    cmds.polyNormalPerVertex(vf_name, normalXYZ=closest_info["normal"][i])
                return
    ...
    def extract_numbers(input_string):
        # 正規表現パターン: 'VERTEX' の後に任意の数のスペース、数字（VERTEXの番号）とコロンが続き、
        # その後任意の数のスペースを無視して数値群をマッチングします。
        pattern = r'VERTEX\s+\d+:\s*(.*)'
        
        # input_string から数値を検索する
        match = re.search(pattern, input_string)
        if match:
            # 空白で文字列を分割してリスト化し、数値のみを抽出して整数型のリストに変換
            numbers = [int(num) for num in re.findall(r'\d+', match.group(1))]
            return numbers
        else:
            # マッチングがない場合は空のリストを返します。
            return []
        ...
        for vertex in current_vertex_info:
            ...
            for current_normal, closest_normal in zip(
                ...
                if not compare_xyz(current_normal, closest_normal):
                    print(current_normal, closest_normal)
