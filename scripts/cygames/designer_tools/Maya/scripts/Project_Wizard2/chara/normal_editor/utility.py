...
# ==================================================
def get_sel_iter(sel_list):

    sel_iter = om2.MItSelectionList(sel_list)
...
def convert_to_vertex(sel_list):
    ...
    for sel_iter in get_sel_iter(sel_list):
        comp = sel_iter.getComponent()

        temp_sel_list = om2.MSelectionList()

        if comp[1].hasFn(om2.MFn.kMeshVertComponent):
            temp_sel_list.add(comp)

        else:
            vertices = cmds.polyListComponentConversion(sel_iter.getStrings(), tv=True)

            for vertex in vertices:
                temp_sel_list.add(vertex)

        if not vert_sel_list.isEmpty():
            temp_sel_list.merge(vert_sel_list, om2.MSelectionList.kRemoveFromList)

        for temp_iter in get_sel_iter(temp_sel_list):
            vert_sel_list.add(temp_iter.getComponent(), False)
