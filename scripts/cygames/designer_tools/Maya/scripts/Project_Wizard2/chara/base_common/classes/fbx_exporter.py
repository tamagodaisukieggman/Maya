...
class FbxExporter(object):
    ...
    def __init__(self):
        ...
        # TODO: fbx_file_pathは「\\」ではエラーが出るため「/」にreplaceしないといけない
    ...
    def export(self):
        ...
        if not cmds.ls(self.target_node_list):
            scenePath = cmds.file(q=True, sn=True)
            cmds.warning("シーン名とマッチするノードがありませんでした: " + str(self.target_node_list) + "がありません。\nシーン: " + scenePath)
            return False
    ...
    def export_fbx_for_model(self):
        ...
        if self.animation_info_list:
            ...
            for anim_info in self.animation_info_list:
                ...
                mel.eval('FBXExportSplitAnimationIntoTakes -v "{0}" {1} {2} ;'.format(this_name, this_start, this_end))
