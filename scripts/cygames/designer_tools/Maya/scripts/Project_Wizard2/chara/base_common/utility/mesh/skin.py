...
def get_skin_cluster(target_transform):
    ...
    this_visibility = cmds.getAttr(
        '{0}.visibility'.format(target_transform))

    if not this_visibility:
        cmds.setAttr('{0}.visibility'.format(target_transform), 1)

    name_for_skin_cluster = target_transform

    split_name = target_transform.split('|')

    if target_transform[0] == '|' and len(split_name) > 2:
        name_for_skin_cluster = target_transform.replace('|', '', 1)

    skin_cluster = mel.eval(
        'findRelatedSkinCluster ' + name_for_skin_cluster)

    cmds.setAttr('{0}.visibility'.format(
        target_transform), this_visibility)
...
def get_skin_root_joint(target_transform):
    ...
    return base_utility.name.get_long_name(first_joint)
...
def get_skin_joint_list(target_transform):
    ...
    for joint in temp_joint_list:
        ...
        long_name = base_utility.name.get_long_name(joint)
...
def set_joint_weight_info_list(target_transform, joint_weight_info_list):
    ...
    for p in range(om_joint_list.length()):
        ...
        this_joint_short_name = \
            base_utility.name.get_short_name(this_joint)
