...
def get_model_info(param_item, arg):
    ...
    joint_count_dict = _count_joint(param_item, CLOTH_JOINT_LIST)
...
def get_mesh_info(param_item, arg):
    ...
    for mesh_param in param_item.main.chara_info.part_info.mesh_param_list:
        ...
        if not mesh_param['check']:
            continue

        mesh = mesh_param['name']

        short_name = mesh.split('|')[-1]
        exists = base_utility.transform.exists(mesh)
        ...
        if exists:
            ...
            this_mesh_dict['value_list'].append('メッシュが存在しません')
...
def get_texture_info(param_item, arg):
    ...
    for texture_param in target_texture_param_list:
        ...
        base_utility.attribute.set_value(
            temp_file_node, 'fileTextureName', texture_path)
