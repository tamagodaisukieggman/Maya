    ...
    for check_dict in check_dict_list:
        ...
        if error_ref_list:
            for error_ref in error_ref_list:
                param_item.error_target_list.append('{0}が{1}ため'.format(error_ref.file_path, error_ref.reason))
...
def check_cloth_joint_sum_count_base(param_item, target_min_count, from_head, arg):
    ...
    for check_dict in check_dict_list:
        ...
        if ref_ctrl.is_error:
            param_item.error_target_list.append('{}が{}ため'.format(ref_ctrl.file_path, ref_ctrl.reason))
            ref_ctrl.unload(unload_hard=True)
            continue
