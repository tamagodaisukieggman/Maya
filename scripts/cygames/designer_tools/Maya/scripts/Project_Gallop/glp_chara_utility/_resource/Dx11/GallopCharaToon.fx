#ifndef _GALLOP_CHARATOON_HLSLINC_
#define _GALLOP_CHARATOON_HLSLINC_
...
#include "GallopCharaToonProps.hlsl"
...
#ifdef USE_FOG
                float3 viewPos = TransformLocalToView(v.vertex);// vpi.positionVS;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float4 colorMask = TEX2D_SAMPLE(_MaskColorTex, i.uv);
                // 0.49～0にかけて_MaskColorR1を、0.51～1にかけて_MaskColorR2を使用する。
                // 0.49～0.51については色は触らない。
                half r1 = (0.49 - min(0.49, colorMask.r)) * (1.0 / 0.49);
                half r2 = (max(0.51, colorMask.r) - 0.51) * (1.0 / 0.49);
                half g1 = (0.49 - min(0.49, colorMask.g)) * (1.0 / 0.49);
                half g2 = (max(0.51, colorMask.g) - 0.51) * (1.0 / 0.49);
                half b1 = (0.49 - min(0.49, colorMask.b)) * (1.0 / 0.49);
                half b2 = (max(0.51, colorMask.b) - 0.51) * (1.0 / 0.49);

                baseColor.rgb *= lerp(float3(1, 1, 1), _MaskColorR1.rgb, r1);
                baseColor.rgb *= lerp(float3(1, 1, 1), _MaskColorR2.rgb, r2);
                baseColor.rgb *= lerp(float3(1, 1, 1), _MaskColorG1.rgb, g1);
                baseColor.rgb *= lerp(float3(1, 1, 1), _MaskColorG2.rgb, g2);
                baseColor.rgb *= lerp(float3(1, 1, 1), _MaskColorB1.rgb, b1);
                baseColor.rgb *= lerp(float3(1, 1, 1), _MaskColorB2.rgb, b2);

#ifdef IMPOSTORS_MASK_COLOR
                float maskR = 0;
                maskR = max(maskR, lerp(0, r1, _R_r1));
                maskR = max(maskR, lerp(0, r2, _R_r2));
                maskR = max(maskR, lerp(0, g1, _R_g1));
                maskR = max(maskR, lerp(0, g2, _R_g2));
                maskR = max(maskR, lerp(0, b1, _R_b1));
                maskR = max(maskR, lerp(0, b2, _R_b2));
                float maskG = 0;
                maskG = max(maskG, lerp(0, r1, _G_r1));
                maskG = max(maskG, lerp(0, r2, _G_r2));
                maskG = max(maskG, lerp(0, g1, _G_g1));
                maskG = max(maskG, lerp(0, g2, _G_g2));
                maskG = max(maskG, lerp(0, b1, _G_b1));
                maskG = max(maskG, lerp(0, b2, _G_b2));
                baseColor = float4(maskR, maskG, 0, 1);
#endif // IMPOSTORS_MASK_COLOR
                ...
                // スフィアマスク
                half reflectionMask = optionMask.r;
#endif

#if defined(MAYA_HLSL)
                //Mayaだと_WorldSpaceCameraPosが取れない
                float3 viewDirection = normalize(half3(ViewInvXf[0].w,ViewInvXf[1].w,ViewInvXf[2].w) - i.posWorld);
#else
                ...
                lightDirection = lerp(normalize(lightData.direction), normalize(_OriginalDirectionalLightDir), step(1, (float)_UseOriginalDirectionalLight));
#else
                lightDirection = lerp(normalize(lightData.direction), normalize(_OriginalDirectionalLightDir), step(1, _UseOriginalDirectionalLight));
#endif
//#endif
                half halfLambert;
#ifdef TOON_HAIR
                ...
                */
                ...
                float4 toonMap = TEX2D_SAMPLE(_ToonMap, i.uv);
#ifdef USE_MASK_COLOR
                toonMap.rgb *= lerp(float3(1, 1, 1), _MaskToonColorR1.rgb, r1);
                toonMap.rgb *= lerp(float3(1, 1, 1), _MaskToonColorR2.rgb, r2);
                toonMap.rgb *= lerp(float3(1, 1, 1), _MaskToonColorG1.rgb, g1);
                toonMap.rgb *= lerp(float3(1, 1, 1), _MaskToonColorG2.rgb, g2);
                toonMap.rgb *= lerp(float3(1, 1, 1), _MaskToonColorB1.rgb, b1);
                toonMap.rgb *= lerp(float3(1, 1, 1), _MaskToonColorB2.rgb, b2);
#ifdef IMPOSTORS_MASK_COLOR
                float TOON_MASK_RATE = 0.7;
                toonMap = float4(maskR * TOON_MASK_RATE, maskG * TOON_MASK_RATE, 0, 1);
#endif // IMPOSTORS_MASK_COLOR

#endif // USE_MASK_COLOR
#if defined(USE_TOONCOLOR)
                //ToonColorの場合はテクスチャを参照せず、BaseColorの輝度値を調整したものをトゥーン色とする
                float3 toonColor = baseColor.rgb * _ToonColor.rgb * lightData.color *_GlobalToonColor.rgb;
#else
                float3 toonColor = toonMap.rgb * lightData.color *_GlobalToonColor.rgb;
#endif
                // 頂点カラーのアルファで補正のかかり具合を調整
                float shadowBias = (1 - ((1 - i.vcolor.a) * _VertexColorToonPower) * (0.5 - abs(min(0, halfLambert - 0.5))) * 2);
                ...
                float4 brightOriginalColor = baseColor;
...
// 眉は独自のかかり方(常にToonの暗い部分の補正がかかる)
                ...
                */
...
#ifdef USE_REFLECTION_MAP
                // リフレクションマップ
                half3 reflectionMapRgb = TEX2D_SAMPLE(_ReflectionMap, euv).rgb;

                float reflectionAddPower = reflectionMapRgb.r * _ReflectionBias; // Rは加算カラーの係数
                float reflectionMulPower = reflectionMapRgb.g * _ReflectionBias; // Gは乗算カラーの割合 (lerpの第三引数)

                // 加算カラー
                half3 reflectionAddcolor = _ReflectionAddColor.rgb;

                // 乗算カラー (shad_cに補正色をかけた後, さらに乗数を増やす)
                half3 reflectionMulColor = toonMap.rgb * _ReflectionMulColor.rgb;
                reflectionMulColor = pow(reflectionMulColor, half3(_ReflectionPowVal, _ReflectionPowVal, _ReflectionPowVal));

                half3 reflectionRgb = (baseColor.rgb + (reflectionAddcolor * reflectionAddPower)) * lerp(float3(1, 1, 1), reflectionMulColor, reflectionMulPower);
                baseColor.rgb = lerp(baseColor.rgb, reflectionRgb, reflectionMask * _ReflectionRate);
#endif
#endif
                ...
                // 彩度
                baseColor.rgb = ColorSaturation(baseColor.rgb, _Saturation);
...
#endif  //_GALLOP_CHARA_TOON_HLSLINC_
