...
#if defined(TOON_REFLECTION)

texture rflTextureMap
<
    string ResourceName = "ctHalf.dds";
    string ResourceType = "2D";
    string UIName =  "Map with dot-half-angle factors";
>;
sampler2D _RflMap = sampler_state
{
    Texture = <rflTextureMap>;
    MinFilter = Linear;
    MagFilter = Linear;
...
float3 _RflAddColor : Ambient <
    string UIWidget = "Color";
> = { 1,1,1 };

float3 _RflMulColor : Ambient <
    string UIWidget = "Color";
> = { 1,1,1 };
...
uniform sampler2D _UVEmissiveTex;
uniform sampler2D _UVEmissiveMaskTex;
