...
#include "GallopCharaToonProps.cginc"
...
// 眉は独自のかかり方(常にToonの暗い部分の補正がかかる)
                ...
                half4 uvEmissiveColor = tex2D(_UVEmissiveTex, uvEmissiveUV);
                half4 uvEmissiveMask = tex2D(_UVEmissiveMaskTex, i.uv);
                baseColor.rgb += (uvEmissiveColor.rgb * _EmissiveColor.rgb) * uvEmissiveMask.r * _UVEmissivePower;
#endif
...
#ifdef TOON_REFLECTION
                //環境マップ2
                half3 rflMaskRgb = tex2D(_RflMap, euv).rgb;

                float rflAddPower = rflMaskRgb.r * _RflBias; // Rは加算
                float rflMulPower = rflMaskRgb.g * _RflBias; // Gは乗算

                half3 rflAddcolor = _RflAddColor.rgb;
                half3 rflMulColor = toonMap.rgb;

                // shad_cの乗数を増やす
                rflMulColor = pow(rflMulColor, half3(_RflPowVal, _RflPowVal, _RflPowVal));

                // shad_cに補正色をかける
                rflMulColor = rflMulColor * _RflMulColor.rgb;

                half3 rfl = (baseColor.rgb + (rflAddcolor * rflAddPower) ) * lerp(float3(1, 1, 1), rflMulColor, rflMulPower);
                baseColor.rgb = lerp(baseColor.rgb, rfl, rflMask * _RflRate);
#endif //TOON_REFLECTION
#endif
