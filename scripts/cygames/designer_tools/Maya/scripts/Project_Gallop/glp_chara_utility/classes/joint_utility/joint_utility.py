...
class SetJointOrient(object):
    ...
    def set_joint_orient(self):
        ...
        if not selected:
            text = '何も選択されていません'
            QtWidgets.QMessageBox.information(None, 'Confirm', text, QtWidgets.QMessageBox.Ok)
            return
        ...
        for sel in selected:
            ...
            if short_name.find('Ear_') == 0 or re.search('Sp_He_Ear[0-9]_', short_name):
    ...
    def set_tail_orient(self):
        ...
        for topNode in topNodes:
            ...
            mbodyNode = topNode + topNode.replace(fileShortNameRemoveExt, 'M_Body')
...
class SpMirror(object):
    ...
    def __get_sp_base_list(self, joint_list):
        ...
        for joint in joint_list:
            ...
            if not this_short_name.startswith(spring_flag):
                continue
            if re.search(ear_flag, this_short_name):
                continue
    ...
    def __get_mirror_path(self, src_sp_path):
        ...
        for path_elm in path_elm_list:
            ...
            if any(ignore_name in path_elm for ignore_name in ignore_str_list):
...
class SpJointCounter(object):
    ...
    def __init__(self):
        ...
        self.sp_joint_regex = re.compile(r'^(Sp|Tp|Ex|Pc)_')
...
class HeadJointMirror(object):
    ...
    def mirror_joints(self, joints):
        ...
        for joint in sorted(joints):
            # Eye_locatorには値が入っているのでrotationを取得
            rot = cmds.xform(joint, q=True, ro=True)
            # YZ面で反転
            rot[1] *= -1
            rot[2] *= -1
            # translate値を取得
            trans = cmds.xform(joint, q=True, ws=True, t=True)
            # xだけ反転
            trans[0] *= -1
            # 置き換える文字を設定 _Rが末尾にあれば_L、_Lが末尾にあれば_Rに置き換える
            replace_str = '_R' if joint.endswith('_L') else '_L'
            reverse_joint = '|'.join([re.sub(r'_[L|R]$', replace_str, s) for s in joint.split('|')])
            # 文字列を置き換えた後に同じ名前だったら処理しない
            if joint == reverse_joint:
                continue
            # 反対側の位置に合わせる
            cmds.xform(reverse_joint, ro=rot)
            cmds.xform(reverse_joint, ws=True, t=trans)

    def mirror_head_joints(self, is_left_to_right):
        """頭部ジョイントをミラーする
        """
        ...
        if not _chara_info.data_type.endswith('head'):
            cmds.warning('頭部のジョイント情報が取得できませんでした')
            return

        all_face_joints = [joint for joint in _chara_info.part_info.joint_list if 'Ear' not in joint] + _chara_info.part_info.locator_list

        selected = cmds.ls(sl=True, l=True)

        if selected:
            selected_joints = set(selected) & set(all_face_joints)

            if not selected_joints:
                cmds.warning('顔のジョイントが選択されていません')
                return

            l_joints = [joint for joint in selected_joints if joint.endswith('_L')]
            r_joints = [joint for joint in selected_joints if joint.endswith('_R')]

            if not l_joints and not r_joints:
                cmds.warning('LとRのジョイントが選択されていません')
                return

            if l_joints and r_joints:
                cmds.warning('LとRのジョイントが選択されています。片側のジョイントを選択してください')
                return

            joints = l_joints if l_joints else r_joints

            self.mirror_joints(joints)

        else:
            end_str = '_L' if is_left_to_right else '_R'

            joints = [joint for joint in all_face_joints if joint.endswith(end_str)]

            self.mirror_joints(joints)


class EarJointMirror(object):
    ...
    def mirror_joints(self, joints):
        ...
        for joint in sorted(joints):
            # ジョイントオリエントを取得
            orient = cmds.joint(joint, q=True, o=True)
            # YZ面で反転
            orient[1] *= -1
            orient[2] *= -1
            # translate値を取得
            ...
            trans[0] *= -1
            # 置き換える文字を設定 _Rが末尾にあれば_L、_Lが末尾にあれば_Rに置き換える
            replace_str = '_R' if re.search(r'_L(?=(_\d{2})?$)', joint) else '_L'
            reverse_joint = '|'.join([re.sub(r'_[LR](?=(_\d{2})?$)', replace_str, s) for s in joint.split('|')])
            # 文字列を置き換えた後に同じ名前だったら処理しない
            if joint == reverse_joint:
                continue
            if not cmds.objExists(reverse_joint):
                continue
            # 反対側の位置に合わせる
            cmds.joint(reverse_joint, e=True, o=orient)
            cmds.xform(reverse_joint, ws=True, t=trans)

    def mirror_ear_joints(self, is_left_to_right):
        """耳のジョイントをミラーする
        """
        ...
        all_ear_joints = [joint for joint in _chara_info.part_info.joint_list if 'Ear' in joint]

        selected = cmds.ls(sl=True, typ='joint', l=True)

        if selected:
            selected_joints = set(selected) & set(all_ear_joints)

            if not selected_joints:
                cmds.warning('耳のジョイントが選択されていません')
                return

            l_joints = [joint for joint in selected_joints if re.search(r'_L(?=(_\d{2})?$)', joint)]
            r_joints = [joint for joint in selected_joints if re.search(r'_R(?=(_\d{2})?$)', joint)]
            ...
            pattern = r'_L(?=(_\d{2})?$)' if is_left_to_right else r'_R(?=(_\d{2})?$)'

            joints = [joint for joint in all_ear_joints if re.search(pattern, joint)]
