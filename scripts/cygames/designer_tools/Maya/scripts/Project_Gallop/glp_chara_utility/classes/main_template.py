...
class Main(object):
    ...
    def __init__(self, main=None, class_name=''):
        """
        """
        ...
        self.this_base_common_reload_str = 'import Project_Gallop.base_common;{}reload(Project_Gallop.base_common);'.format(import_reload_str)
        self.this_glp_common_reload_str = 'import Project_Gallop.glp_common;{}reload(Project_Gallop.glp_common);'.format(import_reload_str)
        ...
        if class_name:
            self.this_class_str = 'Project_Gallop.glp_chara_utility.classes.{}'.format(class_name)
        ...
        self.setting_tool_name = 'GlpCharaUtilityNew'
    ...
    def _create_shelf_command_str(self, exec_module_str, exec_func_str, *exec_func_args):
        ...
        shelf_command = '{0}{1}'.format(
            ...
            self.this_glp_common_reload_str
        )

        if exec_func_args:

            shelf_command += 'import {0}.{1};reload({0}.{1});{0}.{1}.{2}().{3}({4})'.format(
                self.this_class_str,
                exec_module_str,
            ...
            shelf_command += 'import {0}.{1};reload({0}.{1});{0}.{1}.{2}().{3}()'.format(
                self.this_class_str,
                exec_module_str,
                pascal_exec_class_str,
                exec_func_str
    ...
    def _create_shelf_func_command_str(self, exec_module_str, exec_func_str, *exec_func_args):
        ...
        if exec_func_args:
            ...
            shelf_command = 'import {0}.{1};{2}reload({0}.{1});{0}.{1}.{3}({4})'.format(
                self.this_class_str,
                exec_module_str,
                import_reload_str,
                exec_func_str,
            ...
            shelf_command = 'import {0}.{1};{2}reload({0}.{1});{0}.{1}.{3}()'.format(
                self.this_class_str,
                exec_module_str,
                import_reload_str,
                exec_func_str
    ...
    def get_instance_by_name(self, target_module_name):
        """
        """
        ...
        try:
            for load_class in self.main.load_class_list:
                if load_class.__module__.find(target_module_name) >= 0:
                    target_instance = load_class
                    break
        except Exception:
            return None
