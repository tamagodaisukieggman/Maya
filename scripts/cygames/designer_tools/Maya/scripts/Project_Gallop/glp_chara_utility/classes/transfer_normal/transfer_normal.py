...
class TransferNormal(object):
    ...
    def import_template_mesh(self):
        ...
        if not selection:
            text = 'テンプレートのあるメッシュ(例:M_Face, M_Hair)を選択して実行してください'
            QtWidgets.QMessageBox.information(None, 'Usage', text, QtWidgets.QMessageBox.Ok)
            return
    ...
    def set_normal_to_lips(self):

        vertex_faces = cmds.polyListComponentConversion(cmds.ls(sl=True), tvf=True)

        if not vertex_faces:
            cmds.warning('フェースを選択してください')
            return

        cmds.polyNormalPerVertex(vertex_faces, xyz=[0.0, -0.6, 0.8])

    def set_normal_to_mouth_shade(self):

        all_vertex_faces = cmds.polyListComponentConversion(cmds.ls(sl=True), tvf=True)

        if not all_vertex_faces:
            cmds.warning('フェースを選択してください')
            return

        inner_vertices = cmds.ls(cmds.polyListComponentConversion(all_vertex_faces, tv=True, internal=True), fl=True)

        if len(inner_vertices) < 3:
            cmds.warning('選択に中央頂点が含まれていません')
            return

        cmds.polyNormalPerVertex(all_vertex_faces, xyz=[0.0, -0.6, 0.8])

        cmds.polyNormalPerVertex(inner_vertices, xyz=[0.0, -0.675, 0.725])

        sorted_inner_vertices = sorted(inner_vertices, key=lambda v: cmds.pointPosition(v, w=True)[0])
        center_vertex = sorted_inner_vertices[len(sorted_inner_vertices) // 2]
        cmds.polyNormalPerVertex(center_vertex, xyz=[0.0, -0.7, 0.7])
