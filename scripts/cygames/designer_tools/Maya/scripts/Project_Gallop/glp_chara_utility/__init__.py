...
def __reimport_directory(target_dir_path, print_log):
    ...
    if sys.version_info.major == 3:
        reload_str = 'from importlib import reload;'
    ...
    for root, dirs, files in os.walk(target_dir_path):
        ...
        for file_name in files:
            ...
            if is_pyc:
                ...
                exec_string = 'from {0} import {1};{2}reload({1})'.format(
                    relative_path, file_name_noext, reload_str)
        ...
        for dir_name in dirs:
            ...
            # 下層にスクリプトがないフォルダはインポートしない
            has_script = False
            for inner_root, inner_dirs, inner_files in os.walk(os.path.join(root, dir_name)):
                for inner_file in inner_files:
                    if inner_file.endswith('.py') or inner_file.endswith('.pyc'):
                        has_script = True
                        break
            ...
            exec_string = 'from {0} import {1};{2}reload({1})'.format(
                relative_path, dir_name, reload_str)
