...
class Main(object):
    ...
    def snap_data_select_event(self):
        ...
        # jsonの読み取り、一時インポーターを初期化
        json_path = os.path.join(ui.snap_dir_path_line.text(), current_item.text())
        tmp_importer = snap_importer.SnapImporter()
        tmp_importer.initialize(json_path, self.valid_snap_version_range)

        # 一時インポーターが読み取れなかった場合はjsonの選択を戻す
        if not tmp_importer.is_initialized:
        ...
        # 一時インポーターを採用
        self.snap_importer = tmp_importer
