...
class WeightManager(object):
    ...
    def apply_to_skincluster(self, update_dict):
        ...
        for transform in update_dict.keys():
            ...
            mfn_skincluster = utility.get_mfn_skin_clusters_by_transform(transform)
            mfn_skincluster.setWeights(geo_dag_path, vertex_comp, influence_idxs, weights)
    ...
    def __coordinate_data(self, weights, comps, inf_list):
        ...
        filtered_inf_count = len(filtered_inf)

        sorted_weights = [0.0] * len(comps) * filtered_inf_count
        sorted_comps = [0.0] * len(comps)

        remaped_comp = self.__get_order_list(comps)
        for i in range(len(comps)):
            vtx_new_index = remaped_comp[i]
            sorted_comps[vtx_new_index] = comps[i]

            # 無効なindexがない場合はまとめて追加できる
            if not invalid_inf_idx:
                sorted_weights[filtered_inf_count * vtx_new_index:filtered_inf_count * vtx_new_index + filtered_inf_count] = weights[org_inf_count * i:org_inf_count * i + org_inf_count]

            # 無効なindexを含む場合には飛ばして処理
            else:
                j = 0
                for idx in range(org_inf_count):
                    if idx in invalid_inf_idx:
                        continue
                    else:
                        sorted_weights[filtered_inf_count * vtx_new_index + j] = weights[org_inf_count * i + idx]
                        j += 1

        return (sorted_weights, sorted_comps, filtered_inf)

    def __get_order_list(self, target_list):
        """与えられたリストを小さい順に並べなおして自身が何番目に位置するのかを返す

        制約事項:target_list内では重複した要素が与えられた場合最も若いindexが返されます。その際indexが重複します
        ex. [5, 2, 2, 1] => [3, 1, 1, 0]

        Args:
            target_list (list(int|float)): 確認対象の数のリスト

        Returns:
            list(int): 与えられたリストを小さい順に並べなおして自身が何番目に位置するのかのリスト
        """
        sorted_list = sorted(target_list)
        return [sorted_list.index(i) for i in target_list]
