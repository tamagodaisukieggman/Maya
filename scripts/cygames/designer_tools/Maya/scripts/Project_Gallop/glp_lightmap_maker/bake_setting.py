...
def validate_lightset(bake_layer):
    """ライトセットをチェック
    空だと正しくベイクできないため
    ...
    Returns:
        bool: ライトセットがセットアップできているか
    """

    light_set = get_light_set(bake_layer)

    # 空の場合はデフォルトライトセットを調べる
    if not light_set:
        light_set = 'defaultLightSet' if cmds.objExists('defaultLightSet') else None

    if not cmds.sets(light_set, q=True):
        return False

    return True


def validate_layer_members(bake_layer):
    """レイヤーセットメンバーをチェック
    シェイプ以外が混ざっているとturtleコマンドでエラーになるため

    Args:
        bake_layer (str): チェックするベイクレイヤー

    Returns:
        bool: レイヤーセットメンバーが正しいか
    """

    if cmds.sets(bake_layer, q=True) != cmds.sets(bake_layer, q=True, no=True):
        return False  # ノード以外のフェイス等が混ざっている
    
    if not all([cmds.objectType(x) == 'mesh' for x in cmds.sets(bake_layer, q=True)]):
        return False  # メッシュノード以外が混ざっている
