...
def execute(command, wait, **batch_param):
    ...
    if batch_param:
        ...
        for param in list(batch_param.items()):
            ...
            this_env_key = (_g_param_prefix + this_key + _g_param_type_format.format(this_type_str)).upper()
    ...
    # プラグイン読み込みはmaファイルでrequire mtoaが不足して実行できないファイルが発生していたため、当該エラーが起きないようにする暫定対応
    fix_command = 'import os;import maya.cmds as cmds;import Project_Gallop.base_common.utility.simple_batch as glp_simple_bat;glp_simple_bat.load_required_plugins();'
    ...
    if wait:
        subprocess.call([batch_file_path, maya_version, '"' + fix_command + '"'])
    else:
        subprocess.Popen([batch_file_path, maya_version, '"' + fix_command + '"'])
...
def get_param_value(param_name):
    ...
    if param_type_str == 'LIST':
        ...
        param_type = float
        result_value = 0.0
    elif sys.version_info.major == 2 and param_type_str == 'LONG':
        param_type = long
        result_value = long(0.0)
    elif sys.version_info.major == 2 and param_type_str == 'UNICODE':
        param_type = unicode
        result_value = ''
    ...
    for param_value in param_value_list:
        ...
        elif sys.version_info.major == 2 and param_type == long:
            result_value = long(param_value)
        elif sys.version_info.major == 2 and param_type == unicode:
            result_value = unicode(param_value)
