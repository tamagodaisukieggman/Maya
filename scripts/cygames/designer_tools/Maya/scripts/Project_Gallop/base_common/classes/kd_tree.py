...
class KDTree(object):
    ...
    def search_point(self, point):
        ...
        Args:
            point (MPoint|list(int|float)): 探索する位置
        ...
        best = (self.data, self.__get_euclidean_distance(pos, point))
    ...
    def search_radius(self, point, radius):
        ...
        if self.__get_euclidean_distance(pos, point) <= radius:
    ...
    def __get_euclidean_distance(self, arr1, arr2):
        """二つのベクトルのユークリッド距離を計算する

        Args:
            arr1 (list(int|float)): ベクトルが格納されたリスト
            arr2 (list(int|float)): ベクトルが格納されたリスト

        Returns:
            float: arr1とarr2のユークリッド距離
        """
        squared_distance = 0.0
        for i in range(len(arr1)):
            squared_distance += pow(arr1[i] - arr2[i], 2)
