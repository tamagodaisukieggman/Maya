    ...
    def create_all_info(self, option={}):
        ...
        if self.data_info.dress_tail_model_id is not None and self.data_info.dress_tail_model_sub_id is not None:
            ...
            if str(self.data_info.dress_tail_model_id) == '-1':
                ...
                if str(self.data_info.chara_tail_model_id) != '-1':

                    # general_tailを取得する
                    settings = {
                        'data_type': '{0}general_tail'.format(body_type_prefix),
                        'main_id': '{0:04}'.format(self.data_info.chara_tail_model_id),
                        'sub_id': '00',
        ...
        elif self.data_info.chara_tail_model_id is not None and str(self.data_info.chara_tail_model_id) != '-1':
