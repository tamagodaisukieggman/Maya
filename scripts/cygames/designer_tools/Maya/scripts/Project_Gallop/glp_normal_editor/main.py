...
from ..base_common.classes import setting
...
class Main(object):
    ...
    def __init__(self):
        ...
        self.tool_version = '22052001'
        self.tool_name = 'GallopNormalEditor'
    ...
    def display_normal_from_ui(self, show_type):
        ...
        for transform in transform_list:
            ...
            this_shape = cmds.listRelatives(transform, shapes=True, fullPath=True)
    ...
    def list_adjacent_faces(self, component, affect_range=1):
        ...
        """
        faces = []
        vertices = cmds.polyListComponentConversion(component, toVertex=True)
        for _ in range(affect_range):
            vtxs = cmds.polyListComponentConversion(vertices, toVertex=True)
            vtxs = cmds.ls(vtxs, fl=True, l=True)
            for vtx in vtxs:
                current_face = cmds.polyListComponentConversion(vtx, toFace=True)
                faces.extend(current_face)
                vertices.extend(cmds.polyListComponentConversion(current_face, toVertex=True))

        faces = cmds.polyListComponentConversion(faces, toFace=True)
    ...
    def list_adjacent_vertices(self, component, affect_range=1):
        ...
        vertices.extend(cmds.polyListComponentConversion(component, toVertex=True))
        for _ in range(affect_range):
            vtxs = cmds.polyListComponentConversion(vertices, toVertex=True)
            vtxs = cmds.ls(vtxs, fl=True, l=True)
            for vtx in vtxs:
                connected_edges = cmds.polyListComponentConversion(vtx, toEdge=True)
                current_vertexes = cmds.ls(cmds.polyListComponentConversion(connected_edges, toVertex=True), fl=True, l=True)
                vertices.extend(current_vertexes)

        vertices = cmds.polyListComponentConversion(vertices, toVertex=True)
