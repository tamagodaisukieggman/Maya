...
import TkgMultiExporterUI
import TkgCreateExportModel
import TkgOpenWebPage

reload(TkgMultiExporterUI)
reload(TkgCreateExportModel)
reload(TkgOpenWebPage)
...
##################################################
#メインウィンドウクラス
class MainWindow(TkgMultiExporterUI.MainWindow):
    ##########################
    #エクスポート
    ...
    #ヘルプを開く
    def helpFunc(self):
        TkgOpenWebPage.open("https://wisdom.tkgpublic.jp/display/designersmanual/Maya%3A+File#TkgExportModel")
...
def exportOne(currentNode, dstFolderPath, fileType, makedirsFlg=0):
    ...
    #エクスポート用のモデルを作成(1データ)
    resultInfo = TkgCreateExportModel.createOne(currentNode, "export")
