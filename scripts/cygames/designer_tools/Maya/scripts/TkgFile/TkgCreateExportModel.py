...
import TkgDagNode
import TkgDagNode.deleteExtraAttr as deleteExtraAttr

reload(TkgPivotTool)
reload(TkgSelectChildNode)
...
def createAll(transNodes=[]):
    if len(transNodes) == 0:
        #確認メッセージ
        messageStr = u"「TkgExportModel」によってエクスポートされるデータを確認用として作成します。"
...
def applyExportOption(topNode, allNodes, exportOptionInfo):
    ...
    for currentNode in allNodes:
        ...
        if combineFlg == 1:
            ...
            #子メッシュを取得
            childMeshNodes = TkgSelectChildNode.getSpecificTypeChildNodes([currentNode], ["mesh"], 1, 0, 1)
            ...
            if len(childMeshNodes) == 0:
                ...
                #子メッシュが2つ以上ある場合→コンバインする
                    ...
                    TkgPivotTool.moveCenter(currentNewNode, currentWorldPosValue)
...
def getExportOptionInfo(node, allNodes=[], exportOptionInfo={}):
    ...
    #ノードタイプ
    cyDagNode = TkgDagNode.TkgDagNode(node)
