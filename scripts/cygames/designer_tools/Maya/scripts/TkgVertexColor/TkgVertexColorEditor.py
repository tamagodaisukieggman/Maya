...
g_version = '0.1.0'
g_toolName = 'TkgVertexColorEditor'
...
class VertexColorManager(object):
    ...
    def GetVertexList(self):
        ...
        selectList = TkgUtility.GetSelectList()
    ...
    def GetClosestVtxInfo(self, targetPosition, vtxInfoList, maxDistance=5):
        ...
        for p in range(len(vtxInfoList)):
            ...
            thisDistance = TkgVector.GetSqrtDistance(targetPosition, vtxInfoList[p].position)
    ...
    def CreateInfo(self, useCopyList=False):
        ...
        TkgUtility.StartProgress(info)

        for p in range(len(vtxList)):

            if TkgUtility.UpdateProgress(float(p) / float(len(vtxList)), info + '...') is False:
                TkgUtility.EndProgress()
                break
    ...
    def PasteVertexColorByIndex(self):
        ...
        TkgUtility.StartProgress('Paste Vertex Color By Index')

        for p in range(len(self.targetVtxInfoList)):

            if TkgUtility.UpdateProgress(float(p) / float(len(self.targetVtxInfoList)), 'Pasting...') is False:
                TkgUtility.EndProgress()
                break
    ...
    def PasteVertexColorBySelectOrder(self):
        ...
        TkgUtility.StartProgress('Paste Vertex Color By SelectOrder')
    ...
    def PasteVertexColorByPosition(self):
        ...
        TkgUtility.StartProgress('Pasete Vertex Color By Position')
    ...
    def PasteVertexColorByMirror(self, isX, isY, isZ):
        ...
        TkgUtility.StartProgress('Paste Vertex Color By Mirror')
        ...
        for p in range(len(self.targetVtxInfoList)):
            ...
            tempPosition = TkgUtility.CopyList(thisVtxInfo.position)
    ...
    def RoundVertexColor(self, n):
        ...
        TkgUtility.StartProgress('Round Vertex Color')

        for p in range(len(self.targetVtxInfoList)):

            if TkgUtility.UpdateProgress(float(p) / float(len(self.targetVtxInfoList)), 'Rounding...') is False:
                TkgUtility.EndProgress()
                break
...
class VertexInfo(object):
    ...
    def Clone(self):
        ...
        cloneVertexInfo.position = TkgUtility.CopyList(self.position)

        cloneVertexInfo.baseColor = TkgUtility.CopyList(self.baseColor)
    ...
    def CreateInfo(self):
        ...
        self.name = TkgUtility.GetLongName(self.name)

        if cmds.objExists(self.name) is False:
            self.name = ''
            return

        self.index = TkgUtility.GetVertexIndex(self.name)

        self.shortName = TkgUtility.GetShortName(self.name)

        self.meshName = TkgUtility.GetMeshNameFromVertex(self.name)
        self.shortMeshName = TkgUtility.GetShortName(self.meshName)
        ...
        for p in range(0, len(self.vtxFaceInfoList)):
            ...
            if TkgUtility.SameValueList(self.baseColor, thisVtxInfo.color) is False:
    ...
    def RewriteData(self, targetVtxInfo):
        ...
        self.baseColor = TkgUtility.CopyList(targetVtxInfo.baseColor)
        ...
        for p in range(len(self.vtxFaceInfoList)):
            ...
            self.vtxFaceInfoList[p].color = TkgUtility.CopyList(targetVtxInfo.vtxFaceInfoList[thisIndex].color)
...
class VertexFaceInfo(object):
    ...
    def Clone(self, newVertexInfo):
        ...
        cloneVertexFaceInfo.color = TkgUtility.CopyList(self.color)
    ...
    def CreateInfo(self):
        ...
        self.index = TkgUtility.GetVertexFaceIndex(self.name)
