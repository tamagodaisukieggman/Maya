...
import TkgNode
from TkgComponent import getSelPointNames, getPointPosInfo
import TkgSelectChildNode
reload(TkgSelectChildNode)

toolName = "TkgPivotTool"
...
def moveCenter(targetNode, targetWorldPos):
    targetTkgNode = TkgNode.TkgNode(targetNode)

    transformNode = targetTkgNode.transformNode
    shapeNode = targetTkgNode.shapeNode
    nodeType = targetTkgNode.nodeType

    #子ノードを取得
    childNodes = TkgSelectChildNode.getSpecificTypeChildNodes([transformNode], ["all"], 1, 0, 0)
