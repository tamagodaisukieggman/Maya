...
g_toolName = "CySceneOpener"
g_toolVersion = "2.1.2"
g_refecrenceOptionVar = "CySceneOpenerReferenceOptionVar"


class CySceneOpener(QMainWindow):
    ...
    def __init__(self, parent=None):
        super(CySceneOpener, self).__init__(self.get_maya_window())
        self.__ui = Ui_CySceneOpener()
...
class Ui_CySceneOpener(object):
    def setupUi(self, CySceneOpener):
        CySceneOpener.setObjectName("CySceneOpener")
        CySceneOpener.resize(788, 391)
        sizePolicy = QSizePolicy(QSizePolicy.MinimumExpanding, QSizePolicy.MinimumExpanding)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(CySceneOpener.sizePolicy().hasHeightForWidth())
        CySceneOpener.setSizePolicy(sizePolicy)
        self.centralwidget = QWidget(CySceneOpener)
        ...
        CySceneOpener.setCentralWidget(self.centralwidget)
        self.menuBar = QMenuBar(CySceneOpener)
        ...
        CySceneOpener.setMenuBar(self.menuBar)
        self.action_option = QAction(CySceneOpener)
        ...
        self.retranslateUi(CySceneOpener)
        #self.tabWidget.setCurrentIndex(1)
        QMetaObject.connectSlotsByName(CySceneOpener)

    def retranslateUi(self, CySceneOpener):
        try:
            CySceneOpener.setWindowTitle(QApplication.translate("CySceneOpener", "CySceneOpener", None, QApplication.UnicodeUTF8))
            self.menu_setting.setTitle(QApplication.translate("CySceneOpener", "設定", None, QApplication.UnicodeUTF8))
            self.action_option.setText(QApplication.translate("CySceneOpener", "オプション", None, QApplication.UnicodeUTF8))
        except:
            CySceneOpener.setWindowTitle(QApplication.translate("CySceneOpener", "CySceneOpener", None))
            self.menu_setting.setTitle(QApplication.translate("CySceneOpener", "設定", None))
            self.action_option.setText(QApplication.translate("CySceneOpener", "オプション", None))
...
class Widget_Bookmark(QWidget):
    ...
    def loadBookmarkFile(self):
        ...
        if os.path.exists(self.bookmarkFilePath):
            for i, path in enumerate(open(self.bookmarkFilePath, 'r')):
                dirPath = path.rstrip('\n')
    ...
    def saveBookmarkFile(self):
        ...
        ブックマークファイルを保存する
        '''
        file = open(self.bookmarkFilePath, 'w')
