...
def get_setting_path():
    ...
    tool_dir = cmds.internalVar(userAppDir=True) + 'Cygames/' + const.TOOL_NAME + '/'
...
def get_cy_scene_opener_bookmark_file_path():
    """CySceneOpenerのブックマークファイルパスを取得

    Returns:
        str: CySceneOpenerのブックマークファイルパス
    """

    toolDir = cmds.internalVar(userAppDir=True) + 'Cygames/' + const.CY_SCENE_OPENER_NAME + '/'
