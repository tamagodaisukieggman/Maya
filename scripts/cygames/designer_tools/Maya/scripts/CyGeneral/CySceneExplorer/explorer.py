...
class Explorer(object):
    ...
    def load_bookmark(self):
        ...
        if not os.path.exists(utility.get_bookmark_file_path()):
            ...
            # CySceneOpenerのブックマークがあれば読み込む
            load_paths = self.read_cy_scene_opener_bookmark()
    ...
    def read_cy_scene_opener_bookmark(self):
        """CySceneOpenerのブックマークを取得
        初めてツールを起動したときにCySceneOpenerのブックマークを復元する

        Returns:
            [str]: CySceneOpenerのブックマークパスリスト
        """

        load_paths = []
        cy_scene_opener_file_path = utility.get_cy_scene_opener_bookmark_file_path()

        if os.path.exists(cy_scene_opener_file_path):

            confirm = cmds.confirmDialog(
                title='CySceneOpenerからの読み込み',
                message='CySceneOpenerからブックマークを読み込みますか？',
                button=['読み込む', 'キャンセル'],
                defaultButton='読み込む',
                cancelButton='キャンセル',
                dismissString='キャンセル')
