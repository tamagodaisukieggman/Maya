...
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
class OutlineToUVCharaExporterItem(object):

    # ===============================================
    def __init__(self, target_file_path, target_transform_list, root_node, logger, is_ascii, keep_temp_file, show_in_explorer, is_combine_export=False):

        self.target_file_path = target_file_path
        self.target_transform_list = target_transform_list
        ...
        self.temp_file_prefix = '____temp{0:03d}_'.format(self.export_index)
        ...
        self.outline_suffix = '_Outline'
        self.no_export_mesh_suffix_list = [self.outline_suffix]

        # 法線normal値を入れるuvSetの名前
    ...
    def initialize(self):
        ...
        self.output_file_name = '{}.fbx'.format(os.path.splitext(os.path.basename(self.target_file_path))[0])
        if self.is_combine_export:
            self.output_file_name = '{}.fbx'.format(create_full_body.get_full_body_scene_name())

        self.output_file_path = '{}/{}'.format(output_dir, self.output_file_name)
    ...
    def export(self):
        ...
        self.temp_file_path = base_utility.file.open_temp(
            ...
            self.temp_file_prefix + os.path.basename(self.target_file_path)
        )
    ...
    def get_mesh_data(self):
        ...
        mesh_list = []
        for transform in self.target_transform_list:
    ...
    def duplicate_base_mesh_all(self):
        ...
        for mesh_data in self.mesh_data_list:
            ...
            dup_name = short_name + self.duplicate_suffix
    ...
    def completion_outline_object(self, target_transform):
        ...
        if not cmds.objExists(outline_object):
            ...
            dup_obj = cmds.duplicate(target_transform, n='Tmp_Outline')
            cmds.rename(dup_obj[0], outline_short_name)
    ...
    def bake_outline_uv(self, target_transform, src_tarnsform):
        ...
        utility_normal_to_uv.g_logger = self.logger
        utility_normal_to_uv.transfer_normal_to_uvset(
            src_tarnsform,
            target_transform,
            self.uvset_for_normal_xy,
            self.uvset_for_normal_z
        )
    ...
    def optimize_uvset(self, target_transform):
        ...
        if base_utility.mesh.uvset.exists(target_transform, self.uvset_for_normal_xy) and \
                base_utility.mesh.uvset.exists(target_transform, self.uvset_for_normal_z):
        ...
        base_utility.mesh.uvset.change_index(
            target_transform, self.uvset_for_normal_xy, 2
        )

        base_utility.mesh.uvset.change_index(
            target_transform, self.uvset_for_normal_z, 3
        )
    ...
    def optimize_colorset(self, target_transform):
        ...
        base_utility.mesh.colorset.delete(
            target_transform, self.output_colorset
        )
        ...
        base_utility.mesh.colorset.blend(
            target_transform,
            self.output_colorset, first_colorset, 'over'
        )

        base_utility.mesh.colorset.change_index(
            target_transform, self.output_colorset, 0)
        ...
        for this_colorset in colorset_list:
            ...
            if this_colorset == self.output_colorset:
                continue
    ...
    def optimize_skin_mesh(self, target_transform, base_transform):
        ...
        if not base_utility.node.exists(base_transform):
            return
    ...
    def export_fbx(self):
        ...
        self.exporter.is_ascii = self.is_ascii
        self.exporter.target_node_list = [self.root_node]
        self.exporter.fbx_file_path = self.output_file_path

        if not self.exporter.export():
            self.logger.write_log(u'FBXの出力に失敗')
            return

        self.logger.write_log(u'FBXを出力しました')
