...
class ReferenceController(object):
    ...
    def load(self, target_path, target_namespace):
        """
    ...
    def load_using_no_plugin_tmp(self, target_path, target_namespace):
        """プラグイン情報を削除した一時ファイルを作成してロード
        一時ファイルはunloadで削除されます。一時ファイルは.maを直接書き換えているので一時的なジオメトリ参照限定で使用してください。
        """

        # tmpファイルを作成
        tmp_path = target_path.replace('.ma', '__no_plugin_tmp__.ma')
        ...
        self.load(tmp_path, target_namespace)
